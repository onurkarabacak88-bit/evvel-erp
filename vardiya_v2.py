...
def _combine_tarih_saat(
    d: date,
    t_raw: Any,
    *,
    day_default: time = time(9, 0),
) -> datetime:
    """Tarih + time → yerel naif datetime (`sube_operasyon_event.sistem_slot_ts` ile uyumlu)."""
    if t_raw is None:
        return datetime(d.year, d.month, d.day, day_default.hour, day_default.minute, 0)
    if isinstance(t_raw, datetime):
        return datetime(
            ...
            t_raw.hour,
            t_raw.minute,
            getattr(t_raw, "second", 0),
        )
    if isinstance(t_raw, time):
        return datetime(d.year, d.month, d.day, t_raw.hour, t_raw.minute, t_raw.second)
    pt = _parse_saat_metni(t_raw)
    if pt:
        return datetime(d.year, d.month, d.day, pt.hour, pt.minute, pt.second)
    return datetime(d.year, d.month, d.day, day_default.hour, day_default.minute, 0)


def _sube_gun_tip_vardiya_plan(
    cur,
    sube_id: str,
    tarih: date,
    slot_tip: str,
    *,
    ...
    """
    ``vardiya_slot.tip`` (örn. acilis / kapanis) + gün için slot ve isteğe bağlı atama.
    Slot yoksa None — çağıran şube varsayılan saatini kullanır.
    """
    gun = int(tarih.isoweekday())  # 1=Pzt .. 7=Paz — ``vardiya_slot.aktif_gunler``
    cur.execute(
        """
        SELECT
            a.id AS atama_id,
            a.personel_id::text AS plan_personel_id,
            COALESCE(NULLIF(TRIM(p.ad_soyad), ''), a.personel_id::text) AS plan_personel_ad,
            s.id AS slot_id,
            s.ad AS slot_ad,
            s.baslangic_saat AS slot_baslangic,
            s.bitis_saat AS slot_bitis
        FROM vardiya_atama a
        JOIN vardiya_slot s ON s.id = a.slot_id AND s.sube_id = %s
        JOIN personel p ON p.id = a.personel_id
        WHERE a.tarih = %s
          AND a.durum IN ('planli', 'onayli')
          AND s.tip = %s
          AND s.aktif = TRUE
          AND %s = ANY(s.aktif_gunler)
        ORDER BY s.sira NULLS LAST, s.baslangic_saat
        LIMIT 1
        """,
        (sube_id, tarih, slot_tip, gun),
    )
    row = cur.fetchone()
    if row:
        r = dict(row)
        t0 = r.get("slot_baslangic")
        slot_dt = _combine_tarih_saat(tarih, t0, day_default=day_default)
        return {
            "atama_id": r.get("atama_id"),
            "plan_personel_id": (str(r.get("plan_personel_id") or "").strip() or None),
            "plan_personel_ad": (str(r.get("plan_personel_ad") or "").strip() or None),
            "slot_id": r.get("slot_id"),
            "slot_ad": r.get("slot_ad"),
            "slot_baslangic_saat": str(t0) if t0 is not None else None,
            "sistem_slot_dt": slot_dt,
            "kaynak": "vardiya_atama",
        }

    cur.execute(
        """
        SELECT s.id, s.ad, s.baslangic_saat, s.bitis_saat
        FROM vardiya_slot s
        WHERE s.sube_id = %s
          AND s.tip = %s
          AND s.aktif = TRUE
        ...
        LIMIT 1
        """,
        (sube_id, slot_tip, gun),
    )
    row2 = cur.fetchone()
    if not row2:
        return None
    r2 = dict(row2)
    t0 = r2.get("baslangic_saat")
    slot_dt = _combine_tarih_saat(tarih, t0, day_default=day_default)
    return {
        "atama_id": None,
        "plan_personel_id": None,
        "plan_personel_ad": None,
        "slot_id": r2.get("id"),
        "slot_ad": r2.get("ad"),
        "slot_baslangic_saat": str(t0) if t0 is not None else None,
        "sistem_slot_dt": slot_dt,
        "kaynak": "vardiya_slot_only",
    }


def sube_gun_acilis_vardiya_plan(cur, sube_id: str, tarih: date) -> Optional[Dict[str, Any]]:
    """
    Şube + gün için açılış vardiya beklentisi (panel ACILIS zamanı ile hizalar).

    Öncelik: ``vardiya_slot.tip='acilis'`` + o güne ``vardiya_atama`` (planlı/onaylı).
    Atama yoksa: yine açılış tipi slotun ``baslangic_saat`` (personel yok).
    Slot da yoksa: None (çağıran ``sube.acilis_saati`` kullanır).
    """
    return _sube_gun_tip_vardiya_plan(cur, sube_id, tarih, "acilis", day_default=time(9, 0))


def sube_gun_kapanis_vardiya_plan(cur, sube_id: str, tarih: date) -> Optional[Dict[str, Any]]:
    """
    Şube + gün için kapanış vardiya slotu (KAPANIS ``sistem_slot_ts`` ile hizalar).
    Slot yoksa None → ``sube.kapanis_saati``.
    """
    return _sube_gun_tip_vardiya_plan(cur, sube_id, tarih, "kapanis", day_default=time(22, 0))
...
# Haftalık «normal» üst sınır; üzeri planlandığında ek mesai (işletme kuralı).
_VARSAYILAN_MAX_HAFTALIK_TAM_SAAT = 57.0
_VARSAYILAN_MAX_HAFTALIK_PART_SAAT = 33.0


def _max_haftalik_saat_varsayilan(calisma_turu: Optional[str]) -> float:
    """TAM/sürekli: 57h; PART: 33h (üstü ek mesai sayılır)."""
    c = (calisma_turu or "surekli").strip().lower().replace("-", "_")
    if c in ("part_time", "part"):
        return _VARSAYILAN_MAX_HAFTALIK_PART_SAAT
    return _VARSAYILAN_MAX_HAFTALIK_TAM_SAAT


def _personel_kisit_varsayilan(
    personel_id: str,
    *,
    max_gunluk_saat: float = 9.5,
    max_haftalik_saat: float = _VARSAYILAN_MAX_HAFTALIK_TAM_SAAT,
) -> Dict[str, Any]:
    """Şema/kayıt eksik olsa bile tüm beklenen anahtarlar (KeyError önleme)."""
    return {
        "personel_id": personel_id,
        "max_gunluk_saat": float(max_gunluk_saat),
        "max_haftalik_saat": float(max_haftalik_saat),
        "izinli_subeler": [],
        "yasak_subeler": [],
...
def personel_kisit_getir(cur, personel_id: str) -> Dict[str, Any]:
    ...
    _max_gun = _max_gunluk_saat_varsayilan(_cal)
    _max_haf = _max_haftalik_saat_varsayilan(_cal)
    cur.execute(
        "SELECT * FROM personel_kisit WHERE personel_id = %s",
        (personel_id,)
    )
    r = cur.fetchone()
    base = _personel_kisit_varsayilan(
        personel_id,
        max_gunluk_saat=_max_gun,
        max_haftalik_saat=_max_haf,
    )
    if not r:
        return base
    row = dict(r)
    ...
    *,
    ...
    if yeni_toplam > float(kisit['max_gunluk_saat']):
        uyarilar.append({
            ...
            "mesaj": (f"Günlük normal üst limit aşımı (ek mesai): mevcut {gun_d['toplam_saat']:.1f}h + "
    ...
    if yeni_haf > float(kisit['max_haftalik_saat']):
        uyarilar.append({
            ...
            "mesaj": (f"Haftalık normal üst limit aşımı (ek mesai): mevcut {haf:.1f}h + yeni {yeni_sure:.1f}h "
                      f"= {yeni_haf:.1f}h (tanımlı haftalık limit {kisit['max_haftalik_saat']:.1f}h)"),
...
def atama_olustur(
    ...
    *,
    ...
    """, (aid, tarih, slot_id, personel_id, bas, bit, gece, _durum,
...
def personel_ay_vardiya_maas_kaynagi(cur, personel_id: str, yil: int, ay: int) -> Dict[str, Any]:
    ...
    **ek_mesai_haftalik_toplam:** Ay içindeki her Pazartesi ile başlayan hafta için
    ...
    Haftalık limit ``personel_kisit`` üzerinden (TAM 57h / PART 33h varsayılan) okunur.
    """
    ...
    cur.execute(
        ...
        SELECT COALESCE(SUM(EXTRACT(EPOCH FROM
            CASE
                WHEN a.bitis_saat <= a.baslangic_saat
                  THEN ((a.bitis_saat::time + INTERVAL '24 hours') - a.baslangic_saat::time)
                ELSE (a.bitis_saat::time - a.baslangic_saat::time)
            END
        )) / 3600.0, 0) AS saat
        FROM vardiya_atama a
        WHERE a.personel_id = %s
    ...
    return {
        ...
        "ay_baslangic": str(d1),
        "ay_bitis": str(d2),
        "toplam_ay_saat": round(toplam_ay, 2),
        "ek_mesai_haftalik_toplam": round(ek_hafta, 2),
