...
def kasa_kontrol():
    ...
    with db() as (conn, cur):
        ...
        cur.execute("""
            SELECT islem_turu, COUNT(*) as adet, COALESCE(SUM(tutar),0) as toplam
            FROM kasa_hareketleri
            WHERE durum='aktif' AND tarih >= CURRENT_DATE - INTERVAL '7 days'
            GROUP BY islem_turu ORDER BY ABS(SUM(tutar)) DESC
        """)
        son7_ozet = [dict(r) for r in cur.fetchall()]
        cur.execute("""
            SELECT id, tarih, islem_turu, tutar, aciklama
            FROM kasa_hareketleri
            WHERE durum='aktif' AND ABS(tutar) > 10000
            AND tarih >= CURRENT_DATE - INTERVAL '7 days'
            ORDER BY ABS(tutar) DESC LIMIT 20
        """)
...
def toplu_odeme(payload: dict):
    ...
    with db() as (conn, cur):
        ...
        for item in odemeler:
            ...
            if ana_t > 0:
                kaynak = plan.get('kaynak_tablo') or ''
                if kaynak == 'sabit_giderler': islem_t2 = 'SABIT_GIDER'
                elif kaynak == 'personel':     islem_t2 = 'PERSONEL_MAAS'
                else:                          islem_t2 = 'KART_ODEME'
...
if pathlib.Path("static/index.html").exists():
    ...
    async def serve_spa(full_path: str, request: _Req = None):
        """SPA routing — sadece frontend route'ları"""
        import pathlib as _pl
        # Assets ve API buraya düşmemeli — StaticFiles mount'u önce işler
        static_file = _pl.Path("static") / full_path
        if static_file.exists() and static_file.is_file():
            return FileResponse(str(static_file))
        # Diğer tüm route'lar index.html
        return FileResponse(
            "static/index.html",
            headers={"Cache-Control": "no-cache, no-store, must-revalidate",
                     "Pragma": "no-cache", "Expires": "0"}
