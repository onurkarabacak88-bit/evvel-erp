...
def iptal_kasa_hareketi(cur, kaynak_id, kaynak_tablo, islem_turu,
    ...
    """
    # KURAL 1: İptal edilecek kayıt var mı?
    cur.execute("""
    ...
    # Tümünü pasifleştir
    for m in mevcutlar:
        cur.execute("UPDATE kasa_hareketleri SET kasa_etkisi=false WHERE id=%s", (m['id'],))

    # Net tutarı hesapla
    net_tutar = sum(float(m['tutar']) for m in mevcutlar)

    # KURAL 3: Ters kayıt yaz
    _iptal_kasa_etkisi = KASA_ETKISI_MAP.get(iptal_turu, True)
    cur.execute("""
        INSERT INTO kasa_hareketleri
            (id, tarih, islem_turu, tutar, aciklama, kaynak_tablo, kaynak_id, ref_id, ref_type, kasa_etkisi)
        VALUES (%s, CURRENT_DATE, %s, %s, %s, %s, %s, %s, %s, %s)
    """, (str(uuid.uuid4()), iptal_turu, -net_tutar, aciklama,
          kaynak_tablo, kaynak_id, str(uuid.uuid4()), kaynak_tablo.upper(), _iptal_kasa_etkisi))
...
def dis_kaynak_sil(gid: str):
    with db() as (conn, cur):
        ...
        # iptal_kasa_hareketi kaynak_id ile çalışıyor
...
def ciro_sil(cid: str):
    with db() as (conn, cur):
        ...
        # Ledger: ters kayıt yaz (anlik_gider_sil ile aynı model)
