...
def panel():
    try:
        ...
        with db() as (conn, cur):
            ...
            # Anlık gider nakit/kart kırılımı
            cur.execute("""
                SELECT
                    COALESCE(SUM(CASE WHEN odeme_yontemi = 'kart' THEN ABS(tutar) ELSE 0 END), 0) as kart,
                    COALESCE(SUM(CASE WHEN odeme_yontemi != 'kart' OR odeme_yontemi IS NULL THEN ABS(tutar) ELSE 0 END), 0) as nakit
                FROM kasa_hareketleri
                WHERE durum='aktif'
                AND islem_turu = 'ANLIK_GIDER'
                AND EXTRACT(YEAR FROM tarih) = EXTRACT(YEAR FROM CURRENT_DATE)
            ...
            """)
            ag_row = cur.fetchone()
            ozet['bu_ay_anlik_nakit'] = float(ag_row['nakit'])
            ozet['bu_ay_anlik_kart'] = float(ag_row['kart'])
...
@app.get("/api/panel/odeme-yontemi-ozet")
def panel_odeme_yontemi_ozet():
    """Bu ay tüm gider türlerinde nakit vs kart kırılımı — Panel özet kartları için."""
    with db() as (conn, cur):
        bugun = __import__('datetime').date.today()
        yil, ay = bugun.year, bugun.month

        # ANLIK GİDER — kasa_hareketleri
        cur.execute("""
            SELECT
                COALESCE(SUM(CASE WHEN odeme_yontemi = 'kart' THEN ABS(tutar) ELSE 0 END), 0) as kart,
                COALESCE(SUM(CASE WHEN odeme_yontemi != 'kart' OR odeme_yontemi IS NULL THEN ABS(tutar) ELSE 0 END), 0) as nakit
            FROM kasa_hareketleri
            WHERE durum='aktif' AND islem_turu = 'ANLIK_GIDER'
            AND EXTRACT(YEAR FROM tarih) = %s AND EXTRACT(MONTH FROM tarih) = %s
        """, (yil, ay))
        ag = cur.fetchone()

        # SABİT GİDER — kasa_hareketleri
        cur.execute("""
            SELECT
                kh.tarih, ABS(kh.tutar) as tutar,
                COALESCE(kh.odeme_yontemi, 'nakit') as odeme_yontemi,
            ...
            AND EXTRACT(YEAR FROM kh.tarih) = %s AND EXTRACT(MONTH FROM kh.tarih) = %s
        """, (yil, ay))
        sg_rows = cur.fetchall()
        sg_nakit = sum(float(r['tutar']) for r in sg_rows if r['odeme_yontemi'] != 'kart')
        sg_kart  = sum(float(r['tutar']) for r in sg_rows if r['odeme_yontemi'] == 'kart')
        sg_liste = [dict(r) for r in sg_rows]

        # VADELİ — nakit (kasa) + kart (kart_hareketleri)
        cur.execute("""
            SELECT
                kh.tarih, ABS(kh.tutar) as tutar, 'nakit' as odeme_yontemi,
                kh.aciklama
            FROM kasa_hareketleri kh
            WHERE kh.islem_turu = 'VADELI_ODEME' AND kh.kasa_etkisi = true AND kh.durum = 'aktif'
            AND EXTRACT(YEAR FROM kh.tarih) = %s AND EXTRACT(MONTH FROM kh.tarih) = %s
        """, (yil, ay))
        v_nakit_rows = [dict(r) for r in cur.fetchall()]

        cur.execute("""
            SELECT kh.tarih, kh.tutar, 'kart' as odeme_yontemi, kh.aciklama,
                   k.banka, k.kart_adi
            FROM kart_hareketleri kh
            JOIN kartlar k ON k.id = kh.kart_id
            WHERE kh.islem_turu = 'HARCAMA' AND kh.durum = 'aktif'
            AND kh.aciklama ILIKE 'Vadeli alım:%'
            AND EXTRACT(YEAR FROM kh.tarih) = %s AND EXTRACT(MONTH FROM kh.tarih) = %s
        """, (yil, ay))
        v_kart_rows = [dict(r) for r in cur.fetchall()]

        v_nakit = sum(float(r['tutar']) for r in v_nakit_rows)
        v_kart  = sum(float(r['tutar']) for r in v_kart_rows)

        # ANLIK kart liste — kart_hareketleri (odeme_yontemi='kart' olanlar için detay)
        cur.execute("""
            SELECT kh.tarih, ABS(kh.tutar) as tutar, kh.aciklama,
                   COALESCE(kh.odeme_yontemi, 'nakit') as odeme_yontemi
            FROM kasa_hareketleri kh
            WHERE kh.durum='aktif' AND kh.islem_turu = 'ANLIK_GIDER'
            AND EXTRACT(YEAR FROM kh.tarih) = %s AND EXTRACT(MONTH FROM kh.tarih) = %s
            ORDER BY kh.tarih DESC
        """, (yil, ay))
        ag_liste = [dict(r) for r in cur.fetchall()]

        genel_nakit = float(ag['nakit']) + sg_nakit + v_nakit
        genel_kart  = float(ag['kart'])  + sg_kart  + v_kart

        return {
            "anlik":  {"toplam": float(ag['nakit']) + float(ag['kart']), "nakit": float(ag['nakit']), "kart": float(ag['kart']), "liste": ag_liste},
            "sabit":  {"toplam": sg_nakit + sg_kart,  "nakit": sg_nakit,  "kart": sg_kart,  "liste": sg_liste},
            "vadeli": {"toplam": v_nakit + v_kart,    "nakit": v_nakit,   "kart": v_kart,
                       "nakit_liste": v_nakit_rows, "kart_liste": v_kart_rows},
            "genel":  {"nakit_toplam": genel_nakit, "kart_toplam": genel_kart,
                       "toplam": genel_nakit + genel_kart}
        }
...
def odeme_yap(oid: str, tutar: Optional[float] = None, body: VadeliOdeModel = VadeliOdeModel()):
    with db() as (conn, cur):
        ...
        # Kaynak vadeli_alimlar ise tabloyu da odendi yap — çift ödeme engellenir
...
def onayla(oid: str):
    with db() as (conn, cur):
        ...
        if islem_turu == 'ODEME_PLANI':
            ...
            # ÇİFT ÖDEME GUARD: /ode endpoint'i daha önce çalıştıysa kasaya tekrar yazma
            cur.execute("""
            ...
            if cur.fetchone():
                ...
                # Vadeli alım onaylandı — kasa düş
                insert_kasa_hareketi(cur, tarih, islem_turu, signed_tutar,
                    f"Onaylandı: {onay['aciklama']}", onay['kaynak_tablo'], onay['kaynak_id'],
                    ref_id=oid, ref_type='ONAY')
            # vadeli_alimlar tablosunu odendi yap
            cur.execute("""
                UPDATE vadeli_alimlar SET durum='odendi' WHERE id=%s
            """, (onay['kaynak_id'],))
            # Bağlı odeme_plani'nı odendi yap — simülasyondan çıkar
            cur.execute("""
                UPDATE odeme_plani SET durum='odendi', odeme_tarihi=%s
...
def sabit_gider_odemeler(ay: str = None):
    ...
    with db() as (conn, cur):
        ...
        # Nakit/kart kırılımı
        nakit_odenen = sum(float(r['tutar']) for r in odenenler if r.get('odeme_yontemi', 'nakit') != 'kart')
        kart_odenen  = sum(float(r['tutar']) for r in odenenler if r.get('odeme_yontemi') == 'kart')
...
def vadeli_ode(vid: str, body: VadeliOdeModel = VadeliOdeModel()):
    with db() as (conn, cur):
        ...
        """, (vid,))
        if cur.fetchone():
            raise HTTPException(400, "Bu vadeli alım zaten kasaya işlenmiş, tekrar ödeme yapılamaz.")
        ...
        if body.odeme_yontemi == 'kart':
            ...
            cur.execute("""
                INSERT INTO kart_hareketleri
                    (id, kart_id, tarih, islem_turu, tutar, taksit_sayisi, aciklama)
                VALUES (%s, %s, %s, 'HARCAMA', %s, 1, %s)
            """, (hid, body.kart_id, bugun, tutar, f"Vadeli alım: {v['aciklama']}"))
            audit(cur, 'kart_hareketleri', hid, 'VADELI_KART')
            # Plan ve vadeli alımı kapat — kasa etkilenmez
            cur.execute("UPDATE odeme_plani SET durum='odendi', odeme_tarihi=%s, odenen_tutar=%s WHERE id=%s",
                (bugun, tutar, plan['id']))
            cur.execute("""UPDATE onay_kuyrugu SET durum='onaylandi', onay_tarihi=NOW()
                WHERE kaynak_id=%s AND durum NOT IN ('onaylandi','reddedildi')""", (plan['id'],))
    ...
    # NAKİT: mevcut akış — odeme_yap kasaya VADELI_ODEME yazar
    odeme_yap(plan['id'])
    with db() as (conn, cur):
        cur.execute("UPDATE vadeli_alimlar SET durum='odendi' WHERE id=%s", (vid,))
...
def vadeli_kismi_ode(vid: str, body: KismiOdeModel):
    ...
    with db() as (conn, cur):
        ...
        """, (vid,))
        if cur.fetchone():
            raise HTTPException(400, "Bu vadeli alım zaten kasaya işlenmiş.")
...
def vadeli_ozet():
    """Vadeli alımlar özet — Panel kartı için. Nakit + kart kırılımı dahil."""
    with db() as (conn, cur):
        # Bu ay nakit ödenen vadeli alımlar (kasadan çıkan)
        cur.execute("""
            SELECT COALESCE(SUM(ABS(kh.tutar)), 0) as nakit_odenen
            FROM kasa_hareketleri kh
        ...
        """)
        nakit_odenen = float(cur.fetchone()['nakit_odenen'])

        # Bu ay kartla ödenen vadeli alımlar (kart_hareketleri'nden)
        cur.execute("""
            SELECT COALESCE(SUM(kh.tutar), 0) as kart_odenen
            FROM kart_hareketleri kh
            ...
            AND EXTRACT(MONTH FROM kh.tarih) = EXTRACT(MONTH FROM CURRENT_DATE)
        """)
        kart_odenen = float(cur.fetchone()['kart_odenen'])
...
def kismi_odeme_yap(oid: str, body: KismiOdeModel):
    ...
    with db() as (conn, cur):
        ...
        # Kaynak vadeli_alimlar ise tutarı ve vadeyi güncelle
