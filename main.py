...
def panel():
    try:
        ...
        with db() as (conn, cur):
            ...
            # SABİT GİDER — ödeme_plani üzerinden gerçekleşen ödemeler
            # kart_id'li plan = kart talimat, kart_id=NULL ve kaynak=sabit = nakit
            cur.execute("""
                SELECT
                    COALESCE(SUM(CASE WHEN op.kart_id IS NULL THEN op.odenen_tutar ELSE 0 END), 0) as nakit,
                    COALESCE(SUM(CASE WHEN op.kart_id IS NOT NULL THEN op.odenen_tutar ELSE 0 END), 0) as kart
                FROM odeme_plani op
                WHERE op.kaynak_tablo = 'sabit_giderler'
                AND op.durum = 'odendi'
                AND EXTRACT(YEAR FROM op.odeme_tarihi) = EXTRACT(YEAR FROM CURRENT_DATE)
                AND EXTRACT(MONTH FROM op.odeme_tarihi) = EXTRACT(MONTH FROM CURRENT_DATE)
            """)
            sg = cur.fetchone()
            ozet['sabit_nakit'] = float(sg['nakit'])
            ozet['sabit_kart']  = float(sg['kart'])
            ...
            cur.execute("""
                ...
                AND kh.aciklama LIKE 'Vadeli alım:%'
                AND kh.durum = 'aktif'
...
def anlik_gider_ekle(g: AnlikGider):
    with db() as (conn, cur):
        ...
        gid = str(uuid.uuid4())
        cur.execute("""INSERT INTO anlik_giderler (id,tarih,kategori,tutar,aciklama,sube,odeme_yontemi,kart_id)
            VALUES (%s,%s,%s,%s,%s,%s,%s,%s)""",
            (gid, g.tarih, g.kategori, g.tutar, g.aciklama, g.sube, g.odeme_yontemi, g.kart_id))
    ...
    with db() as (conn, cur):
        ...
        if g.gecerlilik_tarihi:
            ...
            cur.execute("""UPDATE sabit_giderler SET gider_adi=%s,kategori=%s,tutar=%s,
                periyot=%s,odeme_gunu=%s,baslangic_tarihi=%s,sube_id=%s,
...
def vadeli_ode(vid: str, body: VadeliOdeModel = VadeliOdeModel()):
    with db() as (conn, cur):
        ...
        # ÇİFT ÖDEME GUARD — sadece tam ödeme yapılmışsa engelle
        # Kısmi ödeme sonrası kasa kaydı olabilir ama vadeli_alimlar 'bekliyor' kalır
        cur.execute("""
            SELECT COUNT(*) as adet, COALESCE(SUM(ABS(tutar)), 0) as toplam_odenen
            FROM kasa_hareketleri
            WHERE kaynak_id=%s AND islem_turu='VADELI_ODEME' AND kasa_etkisi=true AND durum='aktif'
        """, (vid,))
        guard = cur.fetchone()
        if guard and float(guard['toplam_odenen']) >= float(v['tutar']):
...
def vadeli_kismi_ode(vid: str, body: KismiOdeModel):
    ...
    with db() as (conn, cur):
        ...
        # ÇİFT ÖDEME GUARD — kısmi ödeme için tam ödeme yapılmış mı kontrol et
        cur.execute("""
            SELECT COALESCE(SUM(ABS(tutar)), 0) as toplam_odenen
            FROM kasa_hareketleri
            WHERE kaynak_id=%s AND islem_turu='VADELI_ODEME' AND kasa_etkisi=true AND durum='aktif'
        """, (vid,))
        toplam_onceki = float(cur.fetchone()['toplam_odenen'])
        if toplam_onceki >= float(v['tutar']):
            raise HTTPException(400, "Bu vadeli alım zaten tam olarak ödenmiş.")
...
def vadeli_panel_detay():
    ...
    Tüm vadeli alımları (bekliyor + odendi) listeler.
    Her biri için ödeme geçmişini (kart/nakit) döndürür.
    """
    with db() as (conn, cur):
        # Sadece bu ay içinde ödeme yapılan vadeli alımlar
        cur.execute("""
            SELECT DISTINCT
            ...
            WHERE (
                ...
                OR
                -- Bu ay kartla ödeme yapılan
                EXISTS (
                    SELECT 1 FROM kart_hareketleri kh
                    WHERE kh.aciklama ILIKE ('Vadeli alım: ' || va.aciklama || '%')
        ...
        for v in vadeli_liste:
            ...
            cur.execute("""
                ...
                AND (kh.aciklama ILIKE %s OR kh.aciklama ILIKE %s)
                AND EXTRACT(YEAR FROM kh.tarih) = EXTRACT(YEAR FROM CURRENT_DATE)
                AND EXTRACT(MONTH FROM kh.tarih) = EXTRACT(MONTH FROM CURRENT_DATE)
                ORDER BY kh.tarih DESC
            """, (f'Vadeli alım: {v["aciklama"]}%', f'%{v["aciklama"]}%'))
            odemeler += [dict(r) for r in cur.fetchall()]
