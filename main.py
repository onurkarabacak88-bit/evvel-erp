...
def odeme_ertele(oid: str, yeni_tarih: date = None):
    """Ödemeyi ertele — eski onay iptal, yeni tarihle yeni onay açılır."""
    with db() as (conn, cur):
        ...
        # Eski onay kuyruğu kaydını iptal et
        # hem odeme_plani id'si hem kaynak_id ile ara (sabit gider vs kart farklı olabilir)
        cur.execute("""
            UPDATE onay_kuyrugu SET durum='reddedildi'
            WHERE durum='bekliyor'
            AND (kaynak_id=%s OR kaynak_id=(SELECT kaynak_id FROM odeme_plani WHERE id=%s LIMIT 1))
        """, (oid, oid))
        # Yeni tarihle yeni onay kaydı aç
        islem_turu = 'SABIT_GIDER' if o.get('kaynak_tablo') == 'sabit_giderler' else                      'PERSONEL_MAAS' if o.get('kaynak_tablo') == 'personel' else 'ODEME_PLANI'
        onay_ekle(cur,
            islem_turu,
            'odeme_plani', oid,
            f"Ertelendi: {o['aciklama']}",
            float(o['odenecek_tutar']),
            yeni
        )
