...
def kismi_odeme_yap(oid: str, body: KismiOdeModel):
    ...
    with db() as (conn, cur):
        ...
        # Eski plana ait TUM acik onaylari kapat — tum acik durumlar hedeflenir
        cur.execute("""
            UPDATE onay_kuyrugu SET durum='onaylandi', onay_tarihi=NOW()
            WHERE kaynak_id=%s
            AND durum NOT IN ('onaylandi','reddedildi')
        """, (oid,))
        ...
        cur.execute("""
            ...
            VALUES (%s, %s, %s, DATE_TRUNC('month', %s::date), %s, %s, %s, 'onay_bekliyor', %s, %s)
        """, (
            yeni_id,
            plan.get('kart_id'),
