...
def panel():
    try:
        # Panel açıldığında eksik planları kontrol et — eksikse üret
        with db() as (conn, cur):
            # Sabit giderler
            cur.execute("""
            ...
            # Vadeli alımlar — sonradan eklenenler de dahil
            cur.execute("""
                SELECT COUNT(*) as eksik FROM vadeli_alimlar v
                WHERE v.durum = 'bekliyor'
                AND EXTRACT(YEAR FROM v.vade_tarihi) = EXTRACT(YEAR FROM CURRENT_DATE)
                AND EXTRACT(MONTH FROM v.vade_tarihi) = EXTRACT(MONTH FROM CURRENT_DATE)
                AND NOT EXISTS (
                    SELECT 1 FROM odeme_plani op
                    WHERE op.aciklama = 'Vadeli Alım: ' || v.aciklama
                    AND op.durum != 'iptal'
                )
            """)
            eksik_vadeli = cur.fetchone()['eksik']

        if eksik_sabit + eksik_personel + eksik_vadeli > 0:
