...
class SabitGider(BaseModel):
    ...
    kira_artis_tarihi: Optional[date] = None  # Kira artış tarihi
    ...
    with db() as (conn, cur):
        ...
        if g.gecerlilik_tarihi:
            ...
            if g.gecerlilik_tarihi and g.sozlesme_sure_ay:
                from dateutil.relativedelta import relativedelta
                sozlesme_bitis = g.gecerlilik_tarihi + relativedelta(months=g.sozlesme_sure_ay)
            cur.execute("""INSERT INTO sabit_giderler
                (id,gider_adi,kategori,tutar,periyot,odeme_gunu,baslangic_tarihi,sube_id,
                 sozlesme_sure_ay,kira_artis_tarihi,sozlesme_bitis_tarihi)
                VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)""",
                (yeni_id, g.gider_adi, g.kategori, g.tutar, g.periyot,
                 g.odeme_gunu, g.gecerlilik_tarihi, g.sube_id or None,
                 g.sozlesme_sure_ay, g.kira_artis_tarihi, sozlesme_bitis))
            onay_ekle(cur, 'SABIT_GIDER', 'sabit_giderler', yeni_id,
                f"Sabit gider güncellendi: {g.gider_adi}", g.tutar, g.gecerlilik_tarihi)
            ...
            cur.execute("""UPDATE sabit_giderler SET gider_adi=%s,kategori=%s,tutar=%s,
                ...
                (g.gider_adi, g.kategori, g.tutar, g.periyot, g.odeme_gunu,
                 g.baslangic_tarihi, g.sube_id or None, gid))
...
def sabit_gider_uyarilar():
    """Kira/Abonelik sözleşme uyarıları — panel için"""
    from datetime import date, timedelta
    bugun = date.today()
    with db() as (conn, cur):
        cur.execute("""
            SELECT id, gider_adi, kategori, tutar, kira_artis_tarihi, sozlesme_bitis_tarihi
            FROM sabit_giderler
            WHERE aktif = TRUE
    ...
    uyarilar = []
    for r in kayitlar:
        # Kira artış uyarısı — 15 gün öncesinden sarı
        if r['kira_artis_tarihi']:
            ...
            if 0 <= gun_kalan <= 15:
                uyarilar.append({
                    ...
                    'mesaj': f"{r['gider_adi']} için kira artış tarihi {gun_kalan} gün sonra!",
                    'gun_kalan': gun_kalan,
                ...
                uyarilar.append({
                    ...
                    'renk': 'red',
                    'gider_adi': r['gider_adi'],
                    'mesaj': f"{r['gider_adi']} kira artış tarihi geçti! Güncelleme gerekiyor.",
                    'gun_kalan': gun_kalan,
                    'tarih': str(r['kira_artis_tarihi']),
                    'tutar': float(r['tutar'])
                })

        # Sözleşme bitiş uyarısı — 30 gün öncesinden kırmızı
        if r['sozlesme_bitis_tarihi']:
            ...
            if 0 <= gun_kalan <= 30:
                uyarilar.append({
                    ...
                    'renk': 'red',
                    'gider_adi': r['gider_adi'],
                    'mesaj': f"{r['gider_adi']} sözleşmesi {gun_kalan} gün sonra bitiyor!",
                    'gun_kalan': gun_kalan,
...
def aylik_rapor(yil: int = None, ay: int = None):
    ...
    with db() as (conn, cur):
        ...
        cur.execute("""
            ...
            FROM kasa_hareketleri WHERE kasa_etkisi=true AND tarih < %s
        """, (ay_basi,))
