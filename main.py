...
KASA_ETKISI_MAP = {
    ...
    'ODEME_PLANI': True, 'ODEME_IPTAL': True,
...
def insert_kasa_hareketi(cur, tarih, islem_turu, tutar, aciklama,
    ...
    # Bilinmeyen işlem türü → sistem dursun, sessiz hata olmasın
    if islem_turu not in KASA_ETKISI_MAP:
        raise Exception(f"BILINMEYEN_ISLEM_TURU: '{islem_turu}' — KASA_ETKISI_MAP'e ekle")
...
def iptal_kasa_hareketi(cur, kaynak_id, kaynak_tablo, islem_turu,
    ...
    # Bilinmeyen iptal türü → sistem dursun
    if iptal_turu not in KASA_IPTAL_MAP:
        raise Exception(f"BILINMEYEN_IPTAL_TURU: '{iptal_turu}' — KASA_IPTAL_MAP'e ekle")
    _kasa_etkisi = KASA_IPTAL_MAP[iptal_turu]
    cur.execute("""
        INSERT INTO kasa_hareketleri
            (id, tarih, islem_turu, tutar, aciklama, kaynak_tablo, kaynak_id, ref_id, ref_type, kasa_etkisi)
...
# Onay seviye limitleri
ONAY_LIMIT = {
    'KART_ODEME':   0,      # kart ödemesi — her zaman onay
    'SABIT_GIDER':  5000,   # 5.000₺ üstü onay
    'PERSONEL_MAAS':10000,  # 10.000₺ üstü onay
    'ANLIK_GIDER':  2500,   # 2.500₺ üstü onay
    'VADELI_ODEME': 5000,
    'DIS_KAYNAK':   0,      # dış kaynak — her zaman onay
}

def onay_gerekli_mi(islem_turu: str, tutar: float) -> bool:
    """Onay kuyruğuna düşmesi gerekiyor mu?"""
    limit = ONAY_LIMIT.get(islem_turu)
    if limit is None:
        return False   # listede yoksa onay gerekmez
    return tutar >= limit  # limit=0 ise her zaman True

def onay_ekle(cur, islem_turu, kaynak_tablo, kaynak_id, aciklama, tutar, tarih):
    seviye = 'KRITIK' if float(tutar or 0) >= 10000 else 'UYARI' if float(tutar or 0) >= 5000 else 'BILGI'
    cur.execute("""INSERT INTO onay_kuyrugu (id,islem_turu,kaynak_tablo,kaynak_id,aciklama,tutar,tarih,seviye)
        VALUES (%s,%s,%s,%s,%s,%s,%s,%s) ON CONFLICT DO NOTHING""",
        (str(uuid.uuid4()), islem_turu, kaynak_tablo, kaynak_id, aciklama, tutar, tarih, seviye))
        ...
        with db() as (conn, cur):
            cur.execute("""
                SELECT
                    ...
                    -- CIRO_IPTAL negatif tutar, otomatik düşer
                    COALESCE(SUM(CASE WHEN islem_turu IN ('CIRO','CIRO_IPTAL') THEN tutar ELSE 0 END), 0) as sadece_ciro
...
def panel_detay():
    ...
    try:
        ...
        with db() as (conn, cur):
            ...
            cur.execute("""
SELECT
    -- Tüm pozitif hareketler gelir (DEVIR hariç)
    COALESCE(SUM(CASE WHEN tutar > 0 AND islem_turu != 'DEVIR' THEN tutar ELSE 0 END), 0) as gelir,
    -- Tüm negatif hareketler gider (CIRO_IPTAL/DIS_KAYNAK_IPTAL hariç — onlar gelir azalması)
    COALESCE(SUM(CASE WHEN tutar < 0 AND islem_turu NOT IN ('CIRO_IPTAL','DIS_KAYNAK_IPTAL') THEN ABS(tutar) ELSE 0 END), 0) as gider
FROM kasa_hareketleri
WHERE durum='aktif'
""")
...
def odeme_yap(oid: str, tutar: Optional[float] = None):
    with db() as (conn, cur):
        ...
        # Onay kuyruğu kontrolü — bu plan için bekleyen onay varsa bypass etme
        cur.execute("""
            SELECT id FROM onay_kuyrugu
            WHERE kaynak_id = %s AND durum = 'bekliyor'
            LIMIT 1
        """, (oid,))
        bekleyen_onay = cur.fetchone()
        if not bekleyen_onay:
            # kaynak_id üzerinden de bak (sabit gider → plan zinciri)
            kaynak_id = plan.get('kaynak_id')
            if kaynak_id:
                cur.execute("""
                    SELECT id FROM onay_kuyrugu
                    WHERE kaynak_id = %s AND durum = 'bekliyor'
        ...
        if bekleyen_onay:
            raise HTTPException(400,
                "Bu ödeme için onay kuyruğunda bekleyen işlem var. "
                "Lütfen önce Onay Merkezi'nden onaylayın.")
        ...
        if ana_para_kismi > 0:
            ...
            if kaynak == 'sabit_giderler':
                islem_t = 'SABIT_GIDER'
                aciklama_t = f"Sabit gider ödemesi: {plan['aciklama']}"
            elif kaynak == 'personel':
                islem_t = 'PERSONEL_MAAS'
                aciklama_t = f"Personel maaşı: {plan['aciklama']}"
            else:
                islem_t = 'KART_ODEME'
                aciklama_t = f"Kart anapara: {plan['aciklama']}"
...
def onayla(oid: str, tutar: Optional[float] = None):
    """
    Onay kuyruğundaki işlemi onayla ve kasaya yaz.
    tutar: isteğe bağlı — girilirse bu tutarla, girilmezse onay kuyruğundaki tutarla.
    """
    with db() as (conn, cur):
        ...
        # Tutar: kullanıcı girdiyse onu al, yoksa kuyruktaki
        tutar = tutar if tutar is not None else float(onay['tutar'])
        ...
        cur.execute("""UPDATE onay_kuyrugu SET durum='onaylandi', onay_tarihi=NOW(),
            tutar=%s WHERE id=%s AND durum='bekliyor'""", (abs(tutar), oid))
        if cur.rowcount == 0:
            raise HTTPException(409, "Eş zamanlı onay çakışması — işlem zaten onaylandı.")
        # Ödeme planı kaydı varsa ödendi olarak işaretle
        if onay['kaynak_tablo'] == 'odeme_plani' and onay['kaynak_id']:
            cur.execute("""UPDATE odeme_plani SET durum='odendi', odeme_tarihi=%s, odenen_tutar=%s
                WHERE id=%s AND durum='bekliyor'""", (tarih, abs(tutar), onay['kaynak_id']))
        audit(cur, 'onay_kuyrugu', oid, 'ONAYLANDI', eski=onay)
    return {"success": True, "tutar": abs(tutar)}
...
def vadeli_ode(vid: str):
    with db() as (conn, cur):
        ...
        onay_ekle(cur, 'VADELI_ODEME', 'vadeli_alimlar', vid,
            f"Vadeli Ödeme: {v['aciklama']}", v['tutar'], date.today())
...
def kasa_duzelt(sid: str, body: KasaDuzeltModel):
    ...
    with db() as (conn, cur):
        ...
        for k in kayitlar:
            ...
            # 2) Eski CIRO kaydını pasifleştir — immutable model: UPDATE değil iptal+yeni kayıt
            cur.execute("""
                UPDATE kasa_hareketleri SET durum='iptal'
                WHERE id = %s AND durum='aktif'
            """, (k['kasa_id'],))

            # 3) Düzeltilmiş tutarla YENİ CIRO kaydı yaz — geçmiş bozulmaz
            insert_kasa_hareketi(
                cur, k['tarih'], 'CIRO', dogru_tutar,
                f'POS/Online kesinti düzeltmesi (pos:%{pos_oran}, online:%{online_oran})',
                'ciro', k['ciro_id'],
                ref_id=k['ciro_id'],   # aynı ref_id — v_kasa_anomali view çalışmaya devam eder
                ref_type='CIRO'
            )
...
def ledger(limit: int = 200, islem_turu: Optional[str] = None):
    with db() as (conn, cur):
        # Gerçek toplam — limit'ten bağımsız, tüm zamanlar
        ozet_sql = """
            SELECT
                COALESCE(SUM(CASE WHEN tutar > 0 AND islem_turu != 'DEVIR' THEN tutar ELSE 0 END), 0) as toplam_gelir,
                COALESCE(SUM(CASE WHEN tutar < 0 AND islem_turu NOT IN ('CIRO_IPTAL','DIS_KAYNAK_IPTAL') THEN ABS(tutar) ELSE 0 END), 0) as toplam_gider,
                COALESCE(SUM(CASE WHEN islem_turu IN ('CIRO_IPTAL','DIS_KAYNAK_IPTAL') THEN ABS(tutar) ELSE 0 END), 0) as toplam_iptal,
                COALESCE(SUM(tutar), 0) as net
            FROM kasa_hareketleri WHERE durum='aktif' AND kasa_etkisi=true
        """
        if islem_turu:
            ozet_sql += " AND islem_turu=%s"
            cur.execute(ozet_sql, (islem_turu,))
        else:
            cur.execute(ozet_sql)
        ozet = dict(cur.fetchone())

        # Kayıtlar — limite tabi
        ...
        return {"ozet": ozet, "rows": rows, "limit": limit}
                ...
                for row in rows[1:]:
                    ...
                    try:
                        ...
                        if sn == 'ciro':
                            ...
                            if cur.rowcount > 0:
                                # Şube oranlarını çek — manuel ciro ile AYNI net hesap
                                cur.execute("SELECT COALESCE(pos_oran,0) as po, COALESCE(online_oran,0) as oo FROM subeler WHERE id=%s", (sube_id,))
                                oran_row = cur.fetchone()
                                pos_oran_ex    = float(oran_row['po'])    if oran_row else 0.0
                                online_oran_ex = float(oran_row['oo']) if oran_row else 0.0
                                pos_kesinti_ex    = pos    * pos_oran_ex    / 100.0
                                online_kesinti_ex = online * online_oran_ex / 100.0
                                net_tutar_ex = nakit + (pos - pos_kesinti_ex) + (online - online_kesinti_ex)
                                insert_kasa_hareketi(cur, fix_date(d.get('tarih')), 'CIRO',
                                    abs(net_tutar_ex),
                                    f'Excel import - ciro (net) pos:%{pos_oran_ex} online:%{online_oran_ex}',
                                    'ciro', cid, ref_id=cid, ref_type='CIRO')
                                eklenen += 1
                            else:
...
def kasa_kontrol():
    """
    Kasa audit — 3 katmanlı kontrol:
    1. Ciro anomalisi: ciro kaydı var ama kasa karşılığı yok
    2. Kasa tutarlılık: gerçek kasa vs beklenen (ciro - gider)
    3. Günlük sapma: son 7 günde beklenmeyen hareketler
    """
    with db() as (conn, cur):
        # --- 1. Ciro anomalileri ---
        cur.execute("""
        ...
        # --- 2. Gerçek kasa ---
        cur.execute("SELECT COALESCE(SUM(tutar),0) as kasa FROM kasa_hareketleri WHERE kasa_etkisi=true AND durum='aktif'")
        gercek_kasa = float(cur.fetchone()['kasa'])

        # --- 3. Son 7 günde işlem yoğunluğu ve olağandışı hareketler ---
        cur.execute("""
            SELECT
                islem_turu,
                COUNT(*) as adet,
                COALESCE(SUM(tutar),0) as toplam,
                COALESCE(MAX(ABS(tutar)),0) as en_buyuk
            FROM kasa_hareketleri
            WHERE durum='aktif' AND tarih >= CURRENT_DATE - INTERVAL '7 days'
            GROUP BY islem_turu
            ORDER BY ABS(SUM(tutar)) DESC
        """)
        son7_ozet = [dict(r) for r in cur.fetchall()]

        # --- 4. Beklenmeyen büyük hareketler (>10.000₺ tek işlem) ---
        cur.execute("""
            SELECT id, tarih, islem_turu, tutar, aciklama
            FROM kasa_hareketleri
            WHERE durum='aktif'
            AND ABS(tutar) > 10000
            ...
            LIMIT 20
        """)
        buyuk_hareketler = [dict(r) for r in cur.fetchall()]

        # --- 5. Alarm durumu ---
        alarmlar = []
        if sorunlu > 0:
            alarmlar.append({"seviye": "KRITIK", "mesaj": f"{sorunlu} ciro kaydının kasa karşılığı eksik"})
        if gercek_kasa < 0:
            alarmlar.append({"seviye": "KRITIK", "mesaj": f"Kasa negatif: {gercek_kasa:,.0f} ₺"})
        if len(buyuk_hareketler) > 0:
            alarmlar.append({"seviye": "UYARI", "mesaj": f"Son 7 günde {len(buyuk_hareketler)} büyük hareket (>10.000₺)"})

        return {
            "toplam_ciro": toplam,
            "sorunlu": sorunlu,
            "saglikli": toplam - sorunlu,
            "gercek_kasa": gercek_kasa,
            "anomaliler": anomaliler,
            "son7_ozet": son7_ozet,
            "buyuk_hareketler": buyuk_hareketler,
            "alarmlar": alarmlar,
            "alarm_var": len(alarmlar) > 0
        }
...
def toplu_odeme(payload: dict):
    ...
    with db() as (conn, cur):
        ...
        for item in odemeler:
            ...
            # Onay kuyruğu kontrolü — bekleyen onay varsa bu ödemeyi atla
            cur.execute("""
                SELECT id FROM onay_kuyrugu
                WHERE kaynak_id = %s AND durum = 'bekliyor' LIMIT 1
            """, (oid,))
            if cur.fetchone():
                basarili.append({"id": oid, "durum": "onay_bekliyor",
                    "mesaj": "Onay kuyruğunda bekliyor — Onay Merkezi'nden onaylayın"})
                continue
            ...
            if ana_t > 0:
                # kaynak_tablo'ya göre doğru islem_turu — odeme_yap() ile tutarlı
                kaynak = plan.get('kaynak_tablo') or ''
                if kaynak == 'sabit_giderler':
                    islem_t2 = 'SABIT_GIDER'
                elif kaynak == 'personel':
                    islem_t2 = 'PERSONEL_MAAS'
                else:
                    islem_t2 = 'KART_ODEME'
                insert_kasa_hareketi(cur, bugun, islem_t2, -abs(ana_t), plan['aciklama'], 'odeme_plani', oid, oid, 'ODEME_PLANI')
            # Onay kuyruğunu kapat — odeme_plani üzerinden sabit gider onayını da bul
            cur.execute("""UPDATE onay_kuyrugu SET durum='onaylandi', onay_tarihi=NOW()
                WHERE durum='bekliyor'
                AND (
    ...
    with db() as (conn, cur):
        ...
        cur.execute("""
            SELECT
                ...
                -- Gelir: pozitif hareketler (DEVIR hariç, CIRO_IPTAL zaten negatif)
                COALESCE(SUM(CASE WHEN tutar > 0 AND islem_turu != 'DEVIR' THEN tutar ELSE 0 END),0) as toplam_gelir,
                -- Gider: negatif hareketler (CIRO_IPTAL hariç — o gelir azalması, gider değil)
                COALESCE(SUM(CASE WHEN tutar < 0 AND islem_turu NOT IN ('CIRO_IPTAL','DIS_KAYNAK_IPTAL') THEN ABS(tutar) ELSE 0 END),0) as toplam_gider,
                -- İptal edilen gelirler ayrı gösterim için
                COALESCE(SUM(CASE WHEN islem_turu='CIRO_IPTAL' THEN ABS(tutar) ELSE 0 END),0) as ciro_iptal_toplam,
                COALESCE(SUM(CASE WHEN islem_turu='DIS_KAYNAK_IPTAL' THEN ABS(tutar) ELSE 0 END),0) as dis_kaynak_iptal_toplam,
        ...
        # Net ciro = brüt ciro - iptal edilen ciro (gösterim amaçlı, gelir hesabını etkilemez)
        # NOT: toplam_gelir zaten doğru — CIRO_IPTAL negatif olduğu için zaten dahil değil
        # gercek_gelir sadece ciro satırında gösterim için kullanılır
        ozet['net_ciro'] = float(ozet.get('ciro_toplam', 0)) - float(ozet.get('ciro_iptal_toplam', 0))
        ...
        cur.execute("""
            SELECT
                   -- Net ciro: iptal edilenler çıkarılmış
                   COALESCE(SUM(CASE WHEN islem_turu IN ('CIRO','CIRO_IPTAL') THEN tutar ELSE 0 END),0) as ciro,
                   COALESCE(SUM(CASE WHEN tutar>0 AND islem_turu!='DEVIR' THEN tutar ELSE 0 END),0) as gelir,
                   -- Gider: iptal hareketleri hariç
                   COALESCE(SUM(CASE WHEN tutar<0 AND islem_turu NOT IN ('CIRO_IPTAL','DIS_KAYNAK_IPTAL') THEN ABS(tutar) ELSE 0 END),0) as gider
