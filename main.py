...
def startup():
    init_db()
    # 🛟 audit_log aktör kolonları — KENDİ kısa transaction'ında, lock_timeout'lu.
    # Kilit kapanmazsa migrasyon pes eder ve uygulama YİNE AÇILIR; audit() eksik
    # kolona düşerse aktörü JSON gövdesine yazar (bilgi kaybolmaz). Bu ayrımın
    # nedeni database.ensure_audit_aktor docstring'inde yazılı.
    try:
        with db() as (conn, cur):
