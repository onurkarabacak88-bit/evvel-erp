...
def insert_kasa_hareketi(cur, tarih, islem_turu, tutar, aciklama,
    ...
    """
    # Her zaman benzersiz event ID — kaynak_id ile karıştırma
    ...
    """, (str(uuid.uuid4()), str(tarih), islem_turu, tutar, aciklama,
          kaynak_tablo, kaynak_id, _event_id, _ref_type))
...
def iptal_kasa_hareketi(cur, kaynak_id, kaynak_tablo, islem_turu,
    ...
    """
    # KURAL 1: Kayıt var mı? — durum filtresi YOK, kasa_etkisi ile ara
    cur.execute("""
        SELECT id, tutar FROM kasa_hareketleri
        WHERE kaynak_id=%s AND islem_turu=%s AND kasa_etkisi=true
...
def ciro_sil(cid: str):
    with db() as (conn, cur):
        ...
        # Kasa: orijinali iptal et + ters kayıt yaz
        cur.execute("""UPDATE kasa_hareketleri SET durum='iptal'
            WHERE ref_id=%s AND ref_type='CIRO' AND durum='aktif'""", (cid,))

        # Ters kayıt — iptal edilen tutarı geri al
        cur.execute("""SELECT id, tutar FROM kasa_hareketleri
            WHERE ref_id=%s AND ref_type='CIRO'""", (cid,))
        iptal_edilenler = cur.fetchall()
        if iptal_edilenler:
            net_tutar = sum(float(r['tutar']) for r in iptal_edilenler)
            cur.execute("""INSERT INTO kasa_hareketleri
                (id, tarih, islem_turu, tutar, aciklama, kaynak_tablo, kaynak_id, ref_id, ref_type)
                VALUES (%s, CURRENT_DATE, 'CIRO_IPTAL', %s, 'Ciro iptali', 'ciro', %s, %s, 'CIRO')
            """, (str(__import__('uuid').uuid4()), -net_tutar, cid, cid))
