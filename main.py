...
def onayla(oid: str):
    with db() as (conn, cur):
        ...
        if islem_turu == 'ODEME_PLANI':
            ...
            # Vadeli alım onaylandı — kasa düş, vadeli_alimlar ve odeme_plani güncelle
...
def vadeli_ode(vid: str):
    with db() as (conn, cur):
        ...
        # Onay kuyruğuna gönder — onaylandığında kasa düşer, immutable ledger korunur
        cur.execute("""
        ...
        plan = cur.fetchone()
        if plan:
            cur.execute("UPDATE odeme_plani SET durum='onay_bekliyor' WHERE id=%s", (plan['id'],))
        onay_ekle(cur, 'VADELI_ODEME', 'vadeli_alimlar', vid,
            f"Vadeli Alım: {v['aciklama']}", float(v['tutar']), v['vade_tarihi'])
        audit(cur, 'vadeli_alimlar', vid, 'ODE_BEKLIYOR', eski=v)
    return {"success": True, "mesaj": "Onay kuyruğuna gönderildi"}

class VadeliKismiOde(BaseModel):
    odenen_tutar: float
    kalan_vade_tarihi: date

@app.post("/api/vadeli-alimlar/{vid}/kismi-ode")
def vadeli_kismi_ode(vid: str, body: VadeliKismiOde):
    """
    Vadeli alım kısmi ödeme — sabit giderdeki kismi-ode ile aynı mantık:
    1. Kasa sadece ödenen kadar düşer
    2. Vadeli alım tablosunda kalan tutar güncellenir
    3. Bağlı odeme_plani odendi yapılır, kalan için yeni plan oluşur
    4. Motor aynı referans_ay için tekrar plan üretmez
    """
    with db() as (conn, cur):
        cur.execute("SELECT * FROM vadeli_alimlar WHERE id=%s AND durum='bekliyor'", (vid,))
        v = cur.fetchone()
        if not v: raise HTTPException(404, "Kayıt bulunamadı veya zaten ödenmiş")

        toplam = float(v['tutar'])
        ...
        # 1. Kasaya sadece ödenen kadar yaz
        insert_kasa_hareketi(cur, bugun, 'VADELI_ODEME', -abs(odenen),
            f"Kısmi vadeli ödeme: {v['aciklama']} ({int(odenen):,} / {int(toplam):,} ₺)",
            'vadeli_alimlar', vid, vid, 'KISMI_ODE')

        # 2. Bağlı odeme_plani'nı bul — referans_ay'ı koru
        cur.execute("""
            SELECT id, referans_ay FROM odeme_plani
            WHERE kaynak_tablo='vadeli_alimlar' AND kaynak_id=%s
            AND durum IN ('bekliyor','onay_bekliyor')
            LIMIT 1
        """, (vid,))
        plan = cur.fetchone()
        eski_referans_ay = plan['referans_ay'] if plan else bugun

        # 3. Eski planı odendi yap
        if plan:
            cur.execute("""
                UPDATE odeme_plani SET durum='odendi', odeme_tarihi=%s, odenen_tutar=%s
                WHERE id=%s
            """, (bugun, odenen, plan['id']))

        # 4. Eski onay kuyruğunu kapat
        cur.execute("""
            UPDATE onay_kuyrugu SET durum='onaylandi', onay_tarihi=NOW()
            WHERE kaynak_id=%s AND durum NOT IN ('onaylandi','reddedildi')
        """, (vid,))

        # 5. Vadeli alım tablosunda kalan tutarı güncelle
        cur.execute("""
            UPDATE vadeli_alimlar SET tutar=%s, vade_tarihi=%s WHERE id=%s
        """, (kalan, body.kalan_vade_tarihi, vid))

        # 6. Kalan için yeni odeme_plani oluştur — referans_ay aynı kalır
        yeni_pid = str(uuid.uuid4())
        cur.execute("""
            INSERT INTO odeme_plani
                (id, kart_id, tarih, referans_ay, odenecek_tutar, asgari_tutar, aciklama, durum, kaynak_tablo, kaynak_id)
            VALUES (%s, NULL, %s, %s, %s, %s, %s, 'bekliyor', 'vadeli_alimlar', %s)
        """, (yeni_pid, body.kalan_vade_tarihi, eski_referans_ay,
              kalan, kalan, f"Vadeli Alım: {v['aciklama']} (kalan)", vid))

        audit(cur, 'vadeli_alimlar', vid, 'KISMI_ODE',
              eski={'tutar': toplam}, yeni={'odenen': odenen, 'kalan': kalan})

    return {"success": True, "odenen": odenen, "kalan": kalan}
...
def vadeli_ozet():
    """Vadeli alımlar özet — Panel kartı için"""
    with db() as (conn, cur):
        # Bu ay ödenen vadeli alımlar
        cur.execute("""
            SELECT COALESCE(SUM(ABS(kh.tutar)), 0) as toplam_odenen
            FROM kasa_hareketleri kh
        ...
        """)
        toplam_odenen = float(cur.fetchone()['toplam_odenen'])
...
def kismi_odeme_yap(oid: str, body: KismiOdeModel):
    ...
    with db() as (conn, cur):
        ...
        # referans_ay eski planın referans_ay'ı olmalı — motor aynı ay için tekrar plan üretmesin
        eski_referans_ay = plan.get('referans_ay') or str(body.kalan_vade_tarihi)
        yeni_id = str(uuid.uuid4())
        cur.execute("""
            INSERT INTO odeme_plani
                (id, kart_id, tarih, referans_ay, odenecek_tutar, asgari_tutar, aciklama, durum, kaynak_tablo, kaynak_id)
            VALUES (%s, %s, %s, %s, %s, %s, %s, 'bekliyor', %s, %s)
        """, (
            yeni_id,
            plan.get('kart_id'),
