...
def insert_kasa_hareketi(cur, tarih, islem_turu, tutar, aciklama,
    ...
    """
    Merkezi kasa yazma fonksiyonu — tüm kasa insertleri buradan geçer.
    ref_id + ref_type zorunlu tutulur, eksikse kaynak_id kullanılır.
    Silent fail engellendi: kayıt yazılmazsa exception fırlatır.
    """
    _ref_id = ref_id or kaynak_id
    _ref_type = ref_type or (kaynak_tablo.upper() if kaynak_tablo else 'GENEL')

    # Zaten aktif kayıt var mı? Varsa çakışma değil, gerçekten var demektir
    cur.execute("""
        SELECT id FROM kasa_hareketleri
        WHERE ref_id=%s AND ref_type=%s AND islem_turu=%s AND durum='aktif'
    """, (_ref_id, _ref_type, islem_turu))
    if cur.fetchone():
        return  # Zaten var, tekrar yazma
    ...
    """, (str(uuid.uuid4()), str(tarih), islem_turu, tutar, aciklama,
          kaynak_tablo, kaynak_id, _ref_id, _ref_type))

    if cur.rowcount == 0:
        raise Exception(f"KASA YAZILMADI — {islem_turu} / {_ref_id}")
...
def iptal_kasa_hareketi(cur, kaynak_id, kaynak_tablo, islem_turu,
    ...
    """
    # KURAL 1: İptal edilecek aktif orijinal kayıt var mı?
    cur.execute("""
        ...
        for k in kayitlar:
            ...
            if pos_kesinti > 0.01:
                cur.execute("""
                    ...
                    ON CONFLICT (ref_id, ref_type, islem_turu) DO UPDATE
                        SET tutar = EXCLUDED.tutar, aciklama = EXCLUDED.aciklama, durum = 'aktif'
                """, (
