...
def vadeli_ode(vid: str):
    with db() as (conn, cur):
        ...
        # ÇİFT ÖDEME GUARD: kasada bu vadeli alım için zaten aktif VADELI_ODEME kaydı var mı?
        cur.execute("""
            SELECT id FROM kasa_hareketleri
            WHERE kaynak_id=%s AND islem_turu='VADELI_ODEME' AND kasa_etkisi=true AND durum='aktif'
        """, (vid,))
        ...
        # Onay kuyruğunda bekleyen VADELI_ODEME varsa kapat — /ode ve onay çakışmasın
        cur.execute("""
            UPDATE onay_kuyrugu SET durum='reddedildi'
    ...
    # odeme_plani öde — kasa düşer, plan odendi olur (with db() dışında çağrılır)
    odeme_yap(plan['id'])
    with db() as (conn, cur):
        # vadeli_alimlar tablosunu da odendi yap — sayfadan kaybolur, tekrar ödeme engellenir
...
def vadeli_kismi_ode(vid: str, body: KismiOdeModel):
    """
    Vadeli alım kısmi ödeme — frontend doğrudan bu endpoint'i çağırır.
    Bağlı odeme_plani'nı bulur, oradan kismi_odeme_yap'ı tetikler.
    Kural: kasa sadece ödenen kadar etkilenir, kalan yeni plan + yeni vade ile yaşar.
    """
    with db() as (conn, cur):
        cur.execute("SELECT * FROM vadeli_alimlar WHERE id=%s AND durum='bekliyor'", (vid,))
        v = cur.fetchone()
        ...
        if not plan:
            raise HTTPException(400, "Bu vadeli alım için ödeme planı bulunamadı")
    # odeme_plani kismi_odeme_yap — with db() dışında çağrılır
