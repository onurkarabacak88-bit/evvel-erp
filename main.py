...
KASA_ETKISI_MAP = {
    'CIRO': True, 'CIRO_IPTAL': True, 'CIRO_DUZELTME': True,
    ...
    'KASA_GIRIS': True, 'KASA_DUZELTME': True, 'POS_KESINTI': True, 'ONLINE_KESINTI': True,
    'DEVIR': False,
}
...
def insert_kasa_hareketi(cur, tarih, islem_turu, tutar, aciklama,
    ...
    if islem_turu not in KASA_ETKISI_MAP:
        raise Exception(f"Bilinmeyen islem_turu: '{islem_turu}' — KASA_ETKISI_MAP'e ekle")
    _kasa_etkisi = KASA_ETKISI_MAP.get(islem_turu, False)
...
def iptal_kasa_hareketi(cur, kaynak_id, kaynak_tablo, islem_turu,
    ...
    if iptal_turu not in KASA_IPTAL_MAP:
        raise Exception(f"Bilinmeyen iptal_turu: '{iptal_turu}' — KASA_IPTAL_MAP'e ekle")
    _kasa_etkisi = KASA_IPTAL_MAP.get(iptal_turu, False)
    cur.execute("""
        INSERT INTO kasa_hareketleri
            (id, tarih, islem_turu, tutar, aciklama, kaynak_tablo, kaynak_id, ref_id, ref_type, kasa_etkisi)
...
def panel():
    try:
        ...
        # Plan üretimi buradan yapılmaz — startup ve manuel endpoint'e bırakılır
        # GET endpoint'inden side effect YASAK (immutable ledger kuralı)
...
def onayla(oid: str):
    with db() as (conn, cur):
        ...
        if islem_turu not in KASA_ETKISI_MAP:
            raise HTTPException(400, f"Bilinmeyen islem_turu: '{islem_turu}' — onay işlenemiyor")
...
class ReddetModel(BaseModel):
    neden: str  # 'hata' veya 'surec_bitti'

@app.post("/api/onay-kuyrugu/{oid}/reddet")
def reddet(oid: str, body: ReddetModel):
    if body.neden not in ('hata', 'surec_bitti'):
        raise HTTPException(400, "neden 'hata' veya 'surec_bitti' olmalı")
    ...
    with db() as (conn, cur):
        ...
        # Onay kuyruğunu reddet
        cur.execute("UPDATE onay_kuyrugu SET durum='reddedildi', onay_tarihi=NOW() WHERE id=%s", (oid,))

        # Bağlı odeme_plani'nı bul
        if onay['kaynak_tablo'] == 'odeme_plani':
            plan_id = onay['kaynak_id']
            cur.execute("SELECT * FROM odeme_plani WHERE id=%s", (plan_id,))
            plan = cur.fetchone()
            kaynak_tablo = plan['kaynak_tablo'] if plan else None
            kaynak_id    = plan['kaynak_id']    if plan else None
        else:
            # Direkt kaynak (sabit_gider, personel vb.)
            kaynak_tablo = onay['kaynak_tablo']
            kaynak_id    = onay['kaynak_id']
            cur.execute("""
                SELECT id FROM odeme_plani
                WHERE kaynak_id=%s AND durum='onay_bekliyor'
            """, (kaynak_id,))
            plan_row = cur.fetchone()
            plan_id = plan_row['id'] if plan_row else None

        # Her iki durumda da plan iptal edilir — simülasyondan ve yük hesabından çıkar
        if plan_id:
            cur.execute("UPDATE odeme_plani SET durum='iptal' WHERE id=%s", (plan_id,))

        # HATA: sadece bu planı iptal et, kaynak açık kalır — gelecek ay üretilir
        # SÜREÇ BİTTİ: kaynağı da kapat — bir daha plan üretilmez
        if body.neden == 'surec_bitti' and kaynak_tablo and kaynak_id:
            if kaynak_tablo == 'sabit_giderler':
                cur.execute("UPDATE sabit_giderler SET aktif=FALSE WHERE id=%s", (kaynak_id,))
                # Varsa diğer bekleyen planları da iptal et
                cur.execute("""
                ...
                cur.execute("UPDATE personel SET aktif=FALSE WHERE id=%s", (kaynak_id,))
                cur.execute("""
                    UPDATE odeme_plani SET durum='iptal'
                ...
                cur.execute("UPDATE borc_envanteri SET aktif=FALSE WHERE id=%s", (kaynak_id,))
                cur.execute("""
                    UPDATE odeme_plani SET durum='iptal'
        ...
        audit(cur, 'onay_kuyrugu', oid, f'REDDEDILDI_{body.neden.upper()}')
    return {"success": True, "neden": body.neden}
...
def personel_cikis(pid: str, neden: str = ""):
    with db() as (conn, cur):
        cur.execute("UPDATE personel SET aktif=FALSE, cikis_tarihi=%s WHERE id=%s",
            (str(date.today()), pid))
        # Personel çıkışında bekleyen maaş planlarını iptal et — simülasyondan çıkar
        cur.execute("""
            UPDATE odeme_plani SET durum='iptal'
...
def sabit_gider_guncelle(gid: str, g: SabitGider):
    with db() as (conn, cur):
        ...
        if g.gecerlilik_tarihi:
            ...
            # Eski sabit gidere ait TÜM bekleyen ödeme planlarını iptal et
            # (sadece o ay değil — geçmiş birikmiş planlar da temizlenmeli)
            cur.execute("""
                UPDATE odeme_plani SET durum='iptal'
...
def sabit_gider_sil(gid: str):
    with db() as (conn, cur):
        ...
        # Kaynağı kapat
        cur.execute("UPDATE sabit_giderler SET aktif=FALSE WHERE id=%s", (gid,))
        # Bu kaynağa ait TÜM bekleyen/onay_bekleyen planları iptal et
        # Kural: iptal olan kayıtlar simülasyona ve yük hesabına girmez
        cur.execute("""
            UPDATE odeme_plani SET durum='iptal'
            WHERE kaynak_tablo='sabit_giderler'
            AND kaynak_id=%s
    ...
    return {"success": True, "iptal_edilen_plan": iptal_sayisi}
...
def borc_sil(bid: str):
    with db() as (conn, cur):
        ...
        # Bu kaynağa ait TÜM bekleyen planları iptal et — simülasyondan çıkar
        cur.execute("""
            UPDATE odeme_plani SET durum='iptal'
...
def kasa_duzelt(sid: str, body: KasaDuzeltModel):
    ...
    with db() as (conn, cur):
        ...
        for k in kayitlar:
            ...
            # Immutable ledger: eski CIRO ve POS_KESINTI kayıtlarını iptal et
            cur.execute("""
            ...
            # Yeni CIRO kaydı yaz — doğru tutarla (UPDATE yasak, yeni kayıt açılır)
            insert_kasa_hareketi(
                cur, k['tarih'], 'CIRO', dogru_tutar,
                f'POS/Online kesinti düzeltmesi (pos:%{pos_oran}, online:%{online_oran})',
                kaynak_tablo='ciro', kaynak_id=k['ciro_id'],
                ref_id=k['ciro_id'], ref_type='CIRO'
            )
