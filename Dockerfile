...
# ⚠️ DERS (2026-07-05, prod kesintisi): elle yazılan COPY listesi maas_service.py +
# avans_service.py'yi atladı → ModuleNotFoundError → uygulama ayağa kalkamadı.
# Yeni modüller otomatik dahil olsun diye joker desene dönüldü; imajı inceltmek
# istenirse ALLOW-LIST değil .dockerignore (test_*.py vb.) kullanılmalı.
COPY *.py ./
