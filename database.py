...
def init_db():
    ...
    with db() as (conn, cur):
        ...
        # Migration: kasa_etkisi kolonu ekle
        cur.execute("""
            DO $$
            BEGIN
                IF NOT EXISTS (SELECT 1 FROM information_schema.columns
                ...
                THEN
                    ...
                    -- Mevcut DEVIR kayıtlarını false yap
        ...
        # Migration: kasa_hareketleri performans indexleri
        cur.execute("""
            DO $$
            BEGIN
                -- Kompozit index: iptal sorgularının tam şekline uygun
                IF NOT EXISTS (
                    SELECT 1 FROM pg_indexes WHERE indexname = 'idx_kasa_kaynak_tur_durum'
                ) THEN
                    CREATE INDEX idx_kasa_kaynak_tur_durum
                    ON kasa_hareketleri (kaynak_id, islem_turu, durum);
                END IF;

                -- Partial index: SUM(tutar) kasa hesabı için sadece aktif kayıtlar
                IF NOT EXISTS (
                    SELECT 1 FROM pg_indexes WHERE indexname = 'idx_kasa_aktif_tutar'
                ) THEN
                    CREATE INDEX idx_kasa_aktif_tutar
                    ON kasa_hareketleri (tutar)
                    WHERE durum = 'aktif';
                END IF;

                -- Tarih indexi: aylık rapor ve panel sorguları için
                IF NOT EXISTS (
                    SELECT 1 FROM pg_indexes WHERE indexname = 'idx_kasa_tarih'
                ) THEN
                    CREATE INDEX idx_kasa_tarih
        ...
        # Mevcut DB'deki eski trigger'ı da kaldır
        ...
        cur.execute("""
            ...
            BEGIN
                IF NEW.durum = 'iptal' AND OLD.durum = 'aktif' THEN
                    -- Tüm ilgili kasa kayıtlarını pasifleştir (CIRO + POS_KESINTI + ONLINE_KESINTI)
                    UPDATE kasa_hareketleri
                    SET durum = 'iptal'
                    WHERE kaynak_id = NEW.id
                    AND durum = 'aktif';
                    -- NOT: Ters kayıt yazılmıyor — backend zaten hallediyor
                END IF;
                RETURN NEW;
            END;
            $$ LANGUAGE plpgsql;
