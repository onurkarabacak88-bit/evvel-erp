    ...
    with db() as (conn, cur):
        ...
        cur.execute("""
            ...
            BEGIN
                ...
                -- Şubenin online oranını da al
                SELECT COALESCE(online_oran, 0) INTO v_online_oran
                FROM subeler WHERE id = NEW.sube_id;
                -- Net tutar: nakit + pos*(1-pos_oran) + online*(1-online_oran)
                v_net_tutar := NEW.nakit
                    + NEW.pos    * (1 - v_pos_oran/100.0)
                    + NEW.online * (1 - v_online_oran/100.0);
                ...
                END IF;
                -- Online kesintisi
                IF NEW.online > 0 AND v_online_oran > 0 THEN
                    INSERT INTO kasa_hareketleri
                        (id, tarih, islem_turu, tutar, aciklama, kaynak_tablo, kaynak_id, ref_id, ref_type)
                    VALUES
                        (gen_random_uuid()::text, NEW.tarih, 'ONLINE_KESINTI',
                         -(NEW.online * v_online_oran / 100.0),
                         'Online komisyon kesintisi', 'ciro', NEW.id,
                         NEW.id || '_online', 'ONLINE_KESINTI');
                END IF;
                RETURN NEW;
            END;
            $$ LANGUAGE plpgsql;
