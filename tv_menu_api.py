...
VARSAYILAN_KURGU = [
  ...
  {"sure":13500,"e1":{"vitrin":True},"e2":{"carousel":True},"e3":{"vitrin":True}},
    ...
    for idx, s in enumerate(slots):
        ...
        for e in ("e1", "e2", "e3"):
            ...
            if ("menu" not in c and "v" not in c and not c.get("spot")
                    ...
                    and not c.get("carousel")):
                return f"slot {idx}.{e}: menu, klip (v), spot, cok, yeni, sezon, hero, gunun, vitrin, eslesme, oneri, film veya carousel olmalı"
...
.gpr{font-size:2.9vh;text-align:right;white-space:nowrap;font-variant-numeric:tabular-nums}
...
/* ── 🏪 VİTRİN (Greenbox deseni, TULİPİ dili): 3 ürün yan yana, bardak + boy/fiyat ── */
#vitrinRow{display:flex;gap:4vw;justify-content:center;align-items:stretch}
.vkart{flex:1 1 0;min-width:0;display:flex;flex-direction:column;align-items:center;text-align:center;opacity:0;animation:satirBelir .6s ease forwards}
.vcup{height:19vh;max-width:100%;object-fit:contain;filter:drop-shadow(0 2vh 3.6vh rgba(0,0,0,.66))}
.vad{font-size:2.7vh;font-weight:500;color:var(--cream);line-height:1.12;margin-top:2vh}
.vtat{font-size:1.8vh;font-style:italic;color:var(--muted);margin-top:.5vh}
.vpr{display:flex;align-items:baseline;justify-content:center;gap:1.4vw;margin-top:1.1vh;font-variant-numeric:tabular-nums}
.vpr span{font-size:1.8vh;letter-spacing:.14em;text-transform:uppercase;color:var(--muted)}
.vpr b{font-size:2.5vh;font-weight:600;color:var(--cream)}
body.yatay #vitrinRow{gap:3vw;max-width:70vw;margin:0 auto}
body.yatay .vcup{height:26vh}
body.yatay .vad{font-size:3.4vh}
...
#spot.on #spad{animation:satirBelir .7s ease .65s both}
#spot.on #spnot{animation:satirBelir .7s ease .95s both}
#spot.on #spcaz{animation:satirBelir .8s ease 1.2s both}
#spot.on #spfiyat{animation:satirBelir .7s ease 1.45s both}
#spot.on #sppair{animation:satirBelir .7s ease 1.7s both}
body.yatay #spot.duo #spcup{width:24vw;max-width:24vw;max-height:54vh}
body.yatay #spcaz{font-size:2.9vh}
...
/* kapanış vinyeti — son slotun son 1.5sn'inde alt bölge kararır: açılışa "zemin hazırlama" (opacity-only) */
#vinyet{position:absolute;left:0;right:0;bottom:0;height:22vh;z-index:5;pointer-events:none;opacity:0;transition:opacity 1.4s ease;background:linear-gradient(180deg,transparent,rgba(5,3,2,.85))}
#vinyet.on{opacity:1}
...
// menü duvarı koreografisi: satır girişleri ekranlar arası 150ms faz farkıyla (E1→E2→E3 soldan sağa açılır)
var FAZ = EKRAN==='e1' ? 0 : EKRAN==='e2' ? 0.15 : 0.3;
...
function icKlipleri(c,push){
  ...
  if(c.film||c.carousel){ push('tulipi_sut'); push('tulipi_serin'); push('tulipi_iced'); push('tulipi_latte'); push('tulipi_espresso'); push('tulipi_grind'); }
}
...
function spotCiz(){
  ...
  var fk=urunBul(IMZA.ad); cazYaz(fk&&fk.kat);
...
// CEZBEDİCİ ALT YAZILAR — kategori başına tek fısıltı (sahip şablonu: ürün altında davet cümlesi)
var CAZIBE={'Classic Coffees':'Günün ilk iyi kararı.','Signature Coffees':'Bir yudum imza.','Milkshakes':'Soğuk, yoğun, ödül gibi.','Mocktails':'Yeşil, ferah, hafif.','Desserts':'Kahvenin en tatlı yoldaşı.'};
function cazYaz(kat){
  var t=(kat&&CAZIBE[kat])||'';
  spcaz.textContent=t; spcaz.style.display=t?'block':'none';
}
...
// ürüne UYGUN klip — video ürünü zaten gösteriyorsa AYRICA bardak bindirilmez (sahip: mantıksız)
function urunKlibi(ad,kat){
  var s=(ad||'')+' '+(kat||'');
  if(/milkshake|frozen/i.test(s)) return 'tulipi_serin';
  if(/ice|iced|buz|cold|mocktail/i.test(s)) return 'tulipi_iced';
  return 'tulipi_latte';
}
// 🥤 KAHRAMAN ÜRÜN — vaat/kanıt: ürünün KENDİ videosu tam ekran konuşur, bardak bindirme YOK,
// fiyat YOK (rampa: fiyat ilk kez günün seçiminde). Dönüş: zemin klip adı (null=veri yok).
function heroCiz(){
...
function vitrinCiz(){
  var t3=(SIG&&SIG.top3)||[], ads=[], i;
  for(i=0;i<t3.length;i++){ if(t3[i]&&t3[i].ad) ads.push(t3[i].ad); }
  if(!ads.length && IMZA && IMZA.ad) ads=[IMZA.ad];
  var kartlar=[];
  ...
  kartlar.forEach(function(k,ix){
    var rows='';
    if(k.u.f8!=null)  rows+='<div class="vpr"><span>8 oz</span><b>'+escq(k.u.f8)+'</b></div>';
...
// tek ürünü film dilinde çiz (kahve→video / milkshake-mocktail→foto). Dönüş: zemin klip adı.
function filmUrunCiz(ad, kicker){
  if(!ad) return null;
  var f=urunBul(ad), kat=f&&f.kat, s=(ad||'')+' '+(kat||'');
  var fotoOlmali=/milkshake|frappe|mocktail/i.test(s);
  spotEl.classList.remove('duo'); spotEl.classList.remove('altta'); spotEl.classList.remove('film');
  spkick.textContent=kicker||'';
  cazYaz(kat); spad.textContent=ad; spnot.textContent=(f&&f.u.aciklama)||'';
  var fy=f&&(f.u.f8!=null?f.u.f8:(f.u.f14!=null?f.u.f14:f.u.fice));
  spfiyat.textContent=(fy!=null?fy:''); spfiyat.style.color='#C8956A';
  sppair.style.display='none';
  if(!fotoOlmali){ spcup.style.display='none'; spotEl.classList.add('altta'); return filmZeminKahve(ad, kat); }
  void spotEl.offsetWidth; spotEl.classList.add('film');
  urunGorselKoy(ad, kat); spcup.style.display='block'; burstYak(kat);
  return filmZemin(ad, kat);
}
function filmCiz(){
  var ad=(SIG&&SIG.en_cok)||(IMZA&&IMZA.ad); if(!ad) return null;
  return filmUrunCiz(ad, (SIG&&SIG.en_cok)?'BU HAFTA EN ÇOK':'TULİPİ İMZASI');
}
// 🎠 CAROUSEL — top-3 ürün sırayla film dilinde döner (wall-clock senkron, ekranlar aynı ürün)
var carSon=-1;
function zeminOynat(klipAd){
  var v=klipAd&&VID[klipAd]; if(!v) return;
  try{v.currentTime=0;}catch(e){} var p=v.play(); if(p&&p.catch)p.catch(function(){});
  v.classList.add('on'); if(aktif&&aktif!==v)aktif.classList.remove('on'); aktif=v;
}
function carGoster(){
  var slot=SLOT[sonSlot]; var c=slot&&slot[EKRAN];
  if(!c||!c.carousel){ return; }   // sahne değişti → dur
  var t=Date.now()%TOPLAM, ic=t-slot._bas;
  var list=(SIG&&SIG.top3?SIG.top3.map(function(x){return x&&x.ad;}):[]).filter(Boolean);
  if(!list.length && IMZA&&IMZA.ad) list=[IMZA.ad];
  if(list.length>3) list=list.slice(0,3);
  if(!list.length){ carTimer=setTimeout(carGoster,400); return; }
  var per=slot.sure/list.length;
  var i=Math.min(list.length-1, Math.floor(ic/per));
  if(i!==carSon){ carSon=i; zeminOynat(filmUrunCiz(list[i], 'EN ÇOK · '+(i+1)+'/'+list.length)); }
  var kalan=(i+1)*per-ic;
  carTimer=setTimeout(carGoster, Math.max(150, Math.min(kalan,450)));
}
...
function menuCizAd(ad, syf){
  ...
  for(ix=0;ix<us.length;ix++){
    ...
    html+='<div class="grow" style="animation-delay:'+(0.26+FAZ+ix*0.05).toFixed(2)+'s">'
    ...
    for(j=0;j<cols.length;j++){
      ...
      else html+='<div class="gpr cbos"></div>';   // boş hücre sessiz kalır (— işareti muhasebe tablosu hissi veriyordu)
    }
    html+='</div>';
  }
  mlist.innerHTML=html;
...
function goster(c){
  if(!c) return;
  clearTimeout(carTimer); carTimer=null;   // önceki carousel döngüsünü durdur (sahne değişti)
  if(c.carousel){
    menuEl.classList.remove('on');
    carSon=-1; carGoster();
    spotEl.classList.add('on'); metinYaz('',''); brand.classList.remove('on');
    return;
  }
  ...
  if(c.spot){
    ...
    if(vb){ try{vb.currentTime=GECEN;}catch(e){} var pb=vb.play(); if(pb&&pb.catch)pb.catch(function(){}); vb.classList.add('on'); if(aktif&&aktif!==vb)aktif.classList.remove('on'); aktif=vb; }
  ...
  if(c.cok || c.yeni || c.vitrin){
    var ok = c.cok ? cokCiz() : c.yeni ? yeniCiz() : vitrinCiz();
    ...
    if(vf){ try{vf.currentTime=GECEN;}catch(e){} var pf=vf.play(); if(pf&&pf.catch)pf.catch(function(){}); vf.classList.add('on'); if(aktif&&aktif!==vf)aktif.classList.remove('on'); aktif=vf; }
  ...
  if(c.sezon){
    ...
    if(vk){ try{vk.currentTime=GECEN;}catch(e){} var pk=vk.play(); if(pk&&pk.catch)pk.catch(function(){}); vk.classList.add('on'); if(aktif&&aktif!==vk)aktif.classList.remove('on'); aktif=vk; }
...
var sonSlot=-1, GECEN=0, beatTimer=null, vinyetTimer=null, carTimer=null;
function tick(){
  var t=Date.now()%TOPLAM, idx=0, j;
  for(j=0;j<SLOT.length;j++){ if(t>=SLOT[j]._bas) idx=j; else break; }
  if(idx!==sonSlot){
    sonSlot=idx;
    GECEN=Math.max(0,(t-SLOT[idx]._bas)/1000);   // slot içi duvar-saati anı → videolar aynı kareye sarılır (ekranlar-arası ayna)
    goster(SLOT[idx][EKRAN]);
