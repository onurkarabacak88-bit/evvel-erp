...
# ─────────────────────────────────────────────────────────────────
# LAZY MIGRATION — endpoint ilk çağrıldığında tablo yoksa anında oluştur.
# database.py'daki init_db migration'ı SAVEPOINT döngüsünde yutulmuş olabilir.
# ─────────────────────────────────────────────────────────────────

_TABLOLAR_KONTROL_EDILDI = False  # process-level cache; restart'ta sıfırlanır

_RAPOR_TABLOLARI_DDL = [
    """
...
def tablolari_garantile(cur: Any, force: bool = False) -> int:
    """Tüm rapor cache tablolarını CREATE IF NOT EXISTS ile garantile.
    Process-level cache — restart'a kadar bir kez çalışır (force=True override)."""
    global _TABLOLAR_KONTROL_EDILDI
    if _TABLOLAR_KONTROL_EDILDI and not force:
        return 0
    olusturuldu = 0
    for ddl in _RAPOR_TABLOLARI_DDL:
        if _safe_exec(cur, ddl):
            olusturuldu += 1
    _TABLOLAR_KONTROL_EDILDI = True
    return olusturuldu
