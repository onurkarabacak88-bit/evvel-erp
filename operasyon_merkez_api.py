...
def ops_kasa_kaynak_duzelt(uyari_id: str, body: KasaKaynakDuzeltmeBody):
    ...
    with db() as (conn, cur):
        ...
        # ── RAPOR CACHE HOOK — kasa fark düzeltildikten sonra özet yenilensin ──
        try:
...
# ─────────────────────────────────────────────────────────────────
# RAPOR CACHE ENDPOINT'LERİ (Seviye 1+2 raporlama hızlandırma)
# - Hızlı okuma: GET /rapor-cache/...
# - Manuel trigger: POST /rapor-cache/yenile (admin/dev)
# - Batch log görüntüleme: GET /rapor-cache/batch-log
# ─────────────────────────────────────────────────────────────────
...
def ops_rapor_cache_gunluk(
    ...
    """Hızlı: rapor_gunluk_sube_ozet tablosundan oku.
    Parametre boşsa bugün için tüm şubeler."""
...
def ops_rapor_cache_aylik_food(
    ...
    """Hızlı: rapor_aylik_food_cost tablosundan oku."""
...
class RaporCacheYenileBody(BaseModel):
    tipi: str = "gunluk"           # 'gunluk' | 'aylik' | 'her_ikisi'
    tarih: Optional[str] = None    # gunluk için (boşsa bugün)
    year_month: Optional[str] = None  # aylik için (boşsa bu ay)
    sube_id: Optional[str] = None  # tek şube için (boşsa tümü)


@router.post("/rapor-cache/yenile")
def ops_rapor_cache_yenile(body: RaporCacheYenileBody = RaporCacheYenileBody()):
    """Manuel cache yenileme — admin/test için.
    Production'da gece scheduler otomatik çalışır."""
    ...
    try:
        ...
        with db() as (conn, cur):
            batch_log_bitir(cur, bid, islenen=sayac if tipi != "aylik" else sayac_ay,
...
def ops_rapor_cache_batch_log(limit: int = Query(20, ge=1, le=200)):
    """Son N batch çalıştırma logu — admin/monitoring için."""
    with db() as (_, cur):
        cur.execute(
            """
