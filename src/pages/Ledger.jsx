...
export default function Ledger() {
  ...
  useEffect(() => {
    api('/ledger?limit=500').then(data => {
      // Yeni format: {ozet, rows} — eski format: doğrudan dizi
  ...
  // Toplam: backend'den (tüm zamanlar, limit'ten bağımsız)
  ...
  return (
    <div className="page">
      ...
      {/* Özet kartlar — backend'den gerçek toplam */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 12, marginBottom: 16 }}>
        {[
          ...
          { label: toplamIptal > 0 ? '🔄 İptal Edilen' : '= Net', val: toplamIptal > 0 ? toplamIptal : toplamGelir - toplamGider, renk: 'var(--yellow)', sub: toplamIptal > 0 ? 'Ciro/Gelir iptali' : 'Gelir − Gider' },
          ...
          <div key={label} className="metric-card" style={{ borderTop: `3px solid ${renk}` }}>
            ...
            {sub && <div className="metric-sub" style={{ fontSize: 10 }}>{sub}</div>}
          </div>
        ))}
      </div>
