...
export function VadeliAlimlar() {
  ...
  async function ode(id){
    if(!confirm('Onay kuyruğuna gönderilecek. Onaylandığında kasadan düşecek. Devam?'))return;
    try{
      await api(`/vadeli-alimlar/${id}/ode`,{method:'POST'});
      toast('Onay kuyruğuna gönderildi');
      load();
    }catch(e){toast(e.message,'red');}
  }
  async function kismiOdemeOnayla(){
    ...
    try{
      await api(`/vadeli-alimlar/${kismiModal.id}/kismi-ode`,{method:'POST',body:{odenen_tutar:odenen,kalan_vade_tarihi:kismiTarih}});
      const kalan = kismiModal.tutar - odenen;
      toast(`${odenen.toLocaleString('tr-TR')} ₺ ödendi, ${kalan.toLocaleString('tr-TR')} ₺ ${new Date(kismiTarih).toLocaleDateString('tr-TR')} tarihine aktarıldı`);
      setKismiModal(null); setKismiTutar(''); setKismiTarih(''); load();
    }catch(e){toast(e.message,'red');}
  }
                ...
                <tr key={v.id}>
                  ...
                  <td>
                    <div className="flex gap-8">
                      ...
                      <button className="btn btn-ghost btn-sm" onClick={()=>{setKismiModal({id:v.id,aciklama:v.aciklama,tutar:parseFloat(v.tutar)});setKismiTutar('');setKismiTarih('');}}>✂ Kısmi</button>
      ...
      {kismiModal && (
        <div className="modal-overlay" onClick={e=>e.target===e.currentTarget&&setKismiModal(null)}>
          <div className="modal" style={{maxWidth:400}}>
            <div className="modal-header"><h3>✂ Kısmi Ödeme</h3><button className="modal-close" onClick={()=>setKismiModal(null)}>✕</button></div>
            <div className="modal-body">
              <div style={{background:'var(--bg3)',borderRadius:8,padding:'10px 14px',marginBottom:16}}>
                <div style={{fontWeight:600,fontSize:12}}>{kismiModal.aciklama}</div>
                <div style={{fontSize:11,color:'var(--text3)',marginTop:4}}>Toplam borç: <strong>{parseInt(kismiModal.tutar).toLocaleString('tr-TR')} ₺</strong></div>
              </div>
              <div className="form-group">
                <label>Şimdi Ödenecek Tutar (₺)</label>
                <input type="number" value={kismiTutar} onChange={e=>setKismiTutar(e.target.value)} placeholder={`0 - ${parseInt(kismiModal.tutar)} arası`} autoFocus/>
                {kismiTutar && <div style={{fontSize:11,color:'var(--text3)',marginTop:4}}>Kalan: <strong>{(kismiModal.tutar - parseFloat(kismiTutar||0)).toLocaleString('tr-TR')} ₺</strong></div>}
              </div>
              <div className="form-group">
            ...
            <div className="modal-footer">
              ...
              <button className="btn btn-primary" disabled={!kismiTutar||!kismiTarih} onClick={kismiOdemeOnayla}>✓ Uygula</button>
            </div>
          </div>
        </div>
      )}
