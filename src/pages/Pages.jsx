...
export function OnayKuyrugu() {
  ...
  async function reddet(id) {
    // Modal aç — sebep seçtirir
    setReddetModal(id);
  }
  async function reddetGonder(neden) {
    try {
      await api(`/onay-kuyrugu/${reddetModal}/reddet`,{method:'POST', body:{neden}});
      toast(neden === 'surec_bitti' ? 'Reddedildi — kaynak kapatıldı' : 'Reddedildi — plan iptal edildi', 'yellow');
      setReddetModal(null); load();
    } catch(e){toast(e.message,'red');}
  }
      ...
      {reddetModal && (
        <div className="modal-overlay" onClick={e=>e.target===e.currentTarget&&setReddetModal(null)}>
          <div className="modal" style={{maxWidth:400}}>
            <div className="modal-header"><h3>Reddetme Sebebi</h3><button className="modal-close" onClick={()=>setReddetModal(null)}>✕</button></div>
            <div className="modal-body" style={{display:'flex',flexDirection:'column',gap:12}}>
              <button className="btn btn-ghost" style={{textAlign:'left',padding:'14px 16px',border:'1px solid var(--border)',borderRadius:8}} onClick={()=>reddetGonder('hata')}>
              ...
              <button className="btn btn-ghost" style={{textAlign:'left',padding:'14px 16px',border:'1px solid var(--red)',borderRadius:8}} onClick={()=>reddetGonder('surec_bitti')}>
                <div style={{fontWeight:600,marginBottom:4,color:'var(--red)'}}>🚫 Süreç Bitti</div>
                <div style={{fontSize:12,color:'var(--text3)'}}>İlişki kesildi. Kaynak kapatılır, bir daha plan üretilmez.</div>
              </button>
            </div>
          </div>
        </div>
