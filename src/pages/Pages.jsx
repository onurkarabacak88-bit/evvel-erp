...
export function SabitGiderler() {
  ...
  const [showModal, setShowModal] = useState(false);
  const [form, setForm] = useState({gider_adi:'',kategori:'Kira',tutar:'',periyot:'aylik',odeme_gunu:1,baslangic_tarihi:'',sube_id:'',gecerlilik_tarihi:'',sozlesme_sure_ay:'',kira_artis_tarihi:''});
  const [duzenleId, setDuzenleId] = useState(null);
  const [msg, setMsg] = useState(null);
          ...
          <tbody>
            {liste.map(g=>(
              <tr key={g.id}>
                ...
                <td>
                  <div className="flex gap-8">
                    <button className="btn btn-ghost btn-sm" onClick={()=>{setForm({gider_adi:g.gider_adi,kategori:g.kategori,tutar:g.tutar,periyot:g.periyot,odeme_gunu:g.odeme_gunu,baslangic_tarihi:g.baslangic_tarihi?.slice(0,10)||'',sube_id:g.sube_id||'',gecerlilik_tarihi:'',sozlesme_sure_ay:g.sozlesme_sure_ay||'',kira_artis_tarihi:g.kira_artis_tarihi?.slice(0,10)||''});setDuzenleId(g.id);setHatalar({});setShowModal(true);}}>✏️</button>
              ...
              <div className="form-row cols-2">
                ...
                {['Kira','Abonelik'].includes(form.kategori) && (
                  <>
                    ...
                    <div className="form-group">
                      <label>📈 Kira Artış Tarihi</label>
                      <input type="date" value={form.kira_artis_tarihi} onChange={e=>setForm({...form,kira_artis_tarihi:e.target.value})}/>
                      <span style={{fontSize:11,color:'var(--text3)'}}>15 gün öncesinde uyarı gelir</span>
                    </div>
                  </>
                )}
              </div>
