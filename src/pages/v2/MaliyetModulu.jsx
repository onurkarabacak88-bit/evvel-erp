 ...
 *
 * Backend (belge_talep_api._zam_kaynak_etiketi) sahip onaylı fatura bağından
 * doğan alarmlarda provenance'ı bu alanın sonuna damgalıyor:
 *   "ATALAY KAHVE · teslimat-fatura bağı (sahip onaylı) — Fatura NPA042"
 * Sebep: alarm tablosunda serbest metin kolonu yok ve şekli değiştirmemek için
 * yeni kolon açılmadı. Ham alanı olduğu gibi basmak tedarikçi adını okunmaz
 * yapıyor ve "kaç tedarikçi" sayımını şişiriyordu (her fatura ayrı metin).
 *
...
export default function MaliyetModulu({ gorunum, onCekmece, onKopru, onToast }) {
  ...
  // Zam Takibi sekmesi varsayılanı: YALNIZ İNCELENMEMİŞLER (sahibin işi onlar).
  // Filtre ekranda yapılır, sunucuya `sadece_yeni` geçilmez — KPI şeridi hem
  // toplamı hem incelenmemişi göstermek zorunda, tek istekle ikisi de elde.
  ...
  // ⚠️ AYRIŞTIRILDI (2026-08-15): eskiden burada `kdvYukle()` de çağrılıyordu —
  // alarm listesi 'fiyat' görünümündeyken KDV verisi ona bedavaya biniyordu.
  // Liste 'zam' sekmesine taşınınca bu bağ YANLIŞ olurdu: zam ekranı KDV
  // çekerdi, fiyat ekranı KDV'siz kalırdı. Her görünüm kendi verisini ister.
  ...
  useEffect(() => {
    ...
    // 'fiyat' = Fiyat & KDV YÖNETİMİ (CRUD). Zam alarmları buradan 'zam'
    // sekmesine taşındı; bu görünümün tek veri ihtiyacı KDV oranları kaldı.
    // (Davranış eskisiyle birebir: kdvOranlar önceden alarmYukle içinden geliyordu.)
    if (gorunum === 'fiyat') kdvYukle();
    if (gorunum === 'zam') alarmYukle();
    if (gorunum === 'tuketim') kontrolYukle();
    if (gorunum === 'vergi') vergiYukle();
  }, [gorunum, ozetYukle, receteYukle, alarmYukle, kdvYukle, kontrolYukle, vergiYukle, gercekYukle]);
  ...
  if (gorunum === 'fiyat') {
    // ⚠️ Bu görünüm artık YALNIZ fiyat/KDV YÖNETİMİDİR (CRUD). Zam alarm listesi
    // ve onun KPI'ları 'zam' sekmesine taşındı — bu yüzden burada alarm yükleme
    // bekleme/hata kapıları YOK: alarm ucu düşse bile fiyat girişi çalışmalı.
    return (
      <>
        ...
        {/* 📈 TAŞINDI (2026-08-15) — zam alarm listesi 'Zam Takibi' sekmesinde.
            Burada KOPYASI DURMAZ: tek satır köprü kalır. İki ekranın aynı listeyi
            göstermesi "hangisi güncel?" sorusunu doğurur ve biri unutulur. */}
        <button
          onClick={() => onKopru?.('__modul:maliyet:zam')}
          style={{
            width: '100%', textAlign: 'left', cursor: 'pointer', fontFamily: 'inherit',
            ...kartYuzey, padding: '13px 16px', marginBottom: 16,
            border: `1px dashed ${R.cizgi3}`, color: R.metin2, fontSize: 12.5,
            display: 'flex', alignItems: 'center', gap: 10,
          }}
        >
          <span style={{ fontSize: 14 }}>📈</span>
          <span style={{ flex: 1, minWidth: 0 }}>
            Ürün bazlı zam listesi <b style={{ color: R.bakirAcik }}>Zam Takibi</b> sekmesine taşındı
          </span>
          <span style={{ color: R.bakir, fontWeight: 700 }}>→</span>
        </button>
      </>
    );
  }

  // ══════════════════ 📈 ZAM TAKİBİ (2026-08-15) ══════════════════════════════
  // Sahip: "bu artışlar ayrı bir sekme gibi düşün — ürün bazlı artışı görmem
  // gerekiyor". Liste 'Fiyat Zinciri'nden BURAYA TAŞINDI (orada tek satır köprü
  // kaldı) — iki ekran aynı listeyi göstermez.
  //
  // Veri: /ops/fiyat-zam-alarmlari (gun=180&limit=60) — TEK istek, TÜM alarmlar.
