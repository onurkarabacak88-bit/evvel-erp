...
/** API `aktif_gunler` (int[] veya nadir string) → sıralı benzersiz 1..7; omit/null → tüm hafta; bilinçli [] kalır */
function normalizeAktifGunler(raw) {
  const tumHafta = () => [1, 2, 3, 4, 5, 6, 7];
  if (raw === null || raw === undefined || raw === '') return tumHafta();
  const arr = Array.isArray(raw)
    ? raw.map((x) => parseInt(String(x), 10))
    : String(raw).replace(/[{}]/g, '').split(/[,\s]+/).map((x) => parseInt(x.trim(), 10));
  const uniq = [...new Set(arr.filter((n) => Number.isFinite(n) && n >= 1 && n <= 7))].sort((a, b) => a - b);
  if (!uniq.length) {
...
/** Tüm hafta değilse kısa gün listesi (matris / tooltip) */
function slotAktifGunlerEtiket(gunler) {
  const g = normalizeAktifGunler(gunler);
  if (g.length === 7) return null;
  return g.map((i) => TR_GUNLER[i - 1].slice(0, 3)).join(' · ');
}
...
/** Slottaki chip: izinli veya kesin (`onayli`) kayıt — sürüklenemez */
function slottanSuruklenebilirMi(p, atama) {
  const d = p?.gun_durumu || {};
  if (d.durum === 'IZINLI') return false;
  if (atama && String(atama.durum || '') === 'onayli') return false;
  return true;
}
...
function atamaBantIleKesisir(a, bandStartMin, bandEndMin) {
  ...
  return ab < bandEndMin && ae > bandStartMin;
}

/** Bu bant içinde kalan atama kesitlerinin özeti (satır başlığıyla uyumlu saatler). */
function atamaListesiOzetiBandKesisim(atamalar, bandStartMin, bandEndMin) {
  if (!atamalar?.length) return null;
  let minS = Infinity;
  let maxE = -Infinity;
  for (const a of atamalar) {
    if (!atamaBantIleKesisir(a, bandStartMin, bandEndMin)) continue;
    const [a0, a1] = _aralikDakikaExtended(a.baslangic_saat, a.bitis_saat);
    const lo = Math.max(a0, bandStartMin);
    const hi = Math.min(a1, bandEndMin);
    if (hi > lo) {
      if (lo < minS) minS = lo;
      if (hi > maxE) maxE = hi;
    }
...
function _saatEtiketiDakika(m) {
  ...
  let day = Math.floor(u / (24 * 60));
  let r = u % (24 * 60);
  // Matris etiketinde 24:00 / 00:00(+1) yerine 23:59 göster
  if (day > 0 && r === 0) {
    day -= 1;
    r = 24 * 60 - 1;
  }
...
export default function VardiyaPlanlamaV2() {
  ...
  /** AUTO slot üret: hangi günler için — { mode:'tek', subeId, subeAd } | { mode:'tum' } */
  const [slotUretGunModal, setSlotUretGunModal] = useState(null);
  ...
  function slotUretFromSube(subeId, subeAd) {
    setSlotUretGunModal({ mode: 'tek', subeId, subeAd });
  }

  function slotUretTumSubelerAc() {
    const list = gunPlani?.subeler || [];
    if (!list.length) {
      setHata('Şube listesi boş.');
      return;
    }
    setSlotUretGunModal({ mode: 'tum' });
  }

  async function slotUretGunOnayla(aktif_gunler) {
    const gunler = normalizeAktifGunler(aktif_gunler);
    if (!gunler.length) {
      window.alert('En az bir gün seçmelisiniz.');
      return;
    }
    const m = slotUretGunModal;
    if (!m) return;

    const gunAciklama = gunler.length === 7
      ? 'tüm hafta'
      : gunler.map((i) => TR_GUNLER[i - 1].slice(0, 3)).join(', ');

    if (m.mode === 'tek') {
      const msg = `${m.subeAd || 'Şube'}: Şube saatlerinden AUTO slot (${gunAciklama}).\n`
        + 'Ataması olan AUTO slot silinmez; kalanları silinip yenilenir. Devam?';
      if (!confirm(msg)) return;
      setSlotUretGunModal(null);
      try {
        setHata('');
    ...
    const list = gunPlani?.subeler || [];
    if (!confirm(`${list.length} şube için AUTO slot (${gunAciklama}); ataması olan AUTO slot korunur. Devam?`)) return;
    setSlotUretGunModal(null);
    setHata('');
    for (const s of list) {
      try {
  ...
  function hucreMetnindenSaatToplami(metin) {
    if (!metin) return 0;
    const txt = String(metin);
    let toplamDk = 0;
    const re = /(\d{1,2}:\d{2})\s*-\s*(\d{1,2}:\d{2})/g;
    let m;
    while ((m = re.exec(txt)) !== null) {
      const b = saatStrToDk(m[1]);
      const e0 = saatStrToDk(m[2]);
      if (b == null || e0 == null) continue;
      let e = e0;
      if (e <= b) e += 24 * 60;
      toplamDk += Math.max(0, e - b);
    }
    return toplamDk / 60;
  }

  function mesaiHesaplaPanel() {
    if (!personelHafta?.gunler?.length || !personelHafta?.satirlar?.length) {
      setMesaiHesap(null);
      return;
    }
    const gunler = personelHafta.gunler;
    const hesap = (personelHafta.satirlar || []).map((row) => {
      const gunlukTanimli = Number(row.max_gunluk_saat) > 0 ? Number(row.max_gunluk_saat) : 9.5;
      const haftalikTanimli = Number(row.max_haftalik_saat) > 0 ? Number(row.max_haftalik_saat) : (gunlukTanimli * 6);
      const gunDetay = gunler.map((iso) => {
        const c = row.gunler?.[iso];
        const izinli = c?.tip === 'izinli';
        const atanan = izinli ? 0 : hucreMetnindenSaatToplami(c?.metin);
        const fark = atanan - gunlukTanimli;
        return { iso, izinli, atanan, fark };
      });
      const haftalikAtanan = gunDetay.reduce((a, x) => a + (Number(x.atanan) || 0), 0);
      const haftalikFark = haftalikAtanan - haftalikTanimli;
      return {
        personel_id: row.personel_id,
        adSoyad: `${row.ad || ''} ${row.soyad || ''}`.trim() || row.ad_soyad || row.personel_id,
        sube: row.sube_ad || '—',
        gunlukTanimli,
        haftalikTanimli,
        haftalikAtanan,
    ...
    setMesaiHesap({ pzt: personelHafta.pazartesi, satirlar: hesap });
  }

  function personelHaftaHucreMetni(c) {
    if (!c) return '—';
    const m = String(c.metin || '—').replace(/\r/g, '').trim();
    return m || '—';
  }
  ...
  function personelHaftaExcel() {
    ...
    const gunBasliklari = gunler.map((g) => `${personelHaftaGunBaslik(g)} (${g.slice(5)})`);
    const ustSatir = [`Personel x Hafta Vardiya (${pzt})`];
    const bilgiSatir = [`Uretim: ${new Date().toLocaleString('tr-TR')}`];
    const bos = [''];
    const hdr = ['Şube', 'Görev', 'Ad Soyad', ...gunBasliklari, 'Kapanış', 'Notlar'];
    const rows = satirlar.map((row) => {
      const adSoyad = [row.ad, row.soyad].filter(Boolean).join(' ').trim();
      return [
        row.sube_ad || '',
        row.gorev || '',
        adSoyad || row.personel_id || '',
        ...gunler.map((g) => personelHaftaHucreMetni(row.gunler?.[g]).replace(/\n/g, ' | ')),
        Number(row.kapanis_sayisi) || 0,
        row.notlar || '',
      ];
    });
    const aoa = [ustSatir, bilgiSatir, bos, hdr, ...rows];
    const ws = XLSX.utils.aoa_to_sheet(aoa);
    ws['!merges'] = [
      { s: { r: 0, c: 0 }, e: { r: 0, c: hdr.length - 1 } },
      { s: { r: 1, c: 0 }, e: { r: 1, c: hdr.length - 1 } },
    ];
    ws['!autofilter'] = { ref: XLSX.utils.encode_range({ s: { r: 3, c: 0 }, e: { r: 3, c: hdr.length - 1 } }) };
    ws['!cols'] = [
      { wch: 18 }, // Şube
      { wch: 14 }, // Görev
      { wch: 20 }, // Ad Soyad
      ...gunler.map(() => ({ wch: 18 })), // Günler
      { wch: 8 },  // Kapanış
      { wch: 28 }, // Notlar
    ];
    ws['!rows'] = [
    ...
    // Destekleyen Excel istemcilerinde başlık satırı sabitlenir.
  ...
  function personelHaftaPdf() {
    ...
    const head = [[
      ...
      'Görev',
      'Ad Soyad',
      ...gunler.map((g) => `${personelHaftaGunBaslik(g)}\n${g.slice(5)}`),
      'Kap.',
      'Notlar',
    ]];
    const body = satirlar.map((row) => [
      row.sube_ad || '',
      row.gorev || '',
      [row.ad, row.soyad].filter(Boolean).join(' ').trim() || row.personel_id || '',
      ...gunler.map((g) => personelHaftaHucreMetni(row.gunler?.[g])),
      String(Number(row.kapanis_sayisi) || 0),
      (row.notlar || '').slice(0, 200),
    ]);
    const doc = new jsPDF({ orientation: 'landscape', unit: 'mm', format: 'a3' });
    doc.setFontSize(12);
    doc.text(`Personel x Hafta Vardiya (${pzt})`, 14, 12);
    doc.setFontSize(9);
    doc.text(`Uretim: ${new Date().toLocaleString('tr-TR')}`, 14, 18);

    const gunBasCol = 3;
    const gunBitCol = gunBasCol + gunler.length - 1;
    autoTable(doc, {
      head,
      body,
      startY: 22,
      ...
      columnStyles: {
        ...
        1: { cellWidth: 19 },
        2: { cellWidth: 28 },
        ...Object.fromEntries(gunler.map((_, i) => [gunBasCol + i, { cellWidth: 24, halign: 'center' }])),
        [gunBitCol + 1]: { cellWidth: 12, halign: 'center' },
        [gunBitCol + 2]: { cellWidth: 46 },
      },
      didParseCell: (data) => {
        if (data.section !== 'body') return;
        const cidx = data.column.index;
        if (cidx >= gunBasCol && cidx <= gunBitCol) {
          const txt = String(data.cell.raw || '').toUpperCase();
          if (txt.includes('İZİNLİ') || txt.includes('IZINLI')) {
      ...
      didDrawPage: (data) => {
        ...
        doc.setFontSize(8);
        doc.text(`Sayfa ${page}`, data.settings.margin.left, doc.internal.pageSize.getHeight() - 6);
      },
    });
  ...
  async function atamaIptal(atamaId, silent = false, durum = null) {
    if (!silent) {
      ...
      const msg = kilit
        ? 'Bu atama Kaydet ile kesinlenmiş (kilitli). İptal etmek istediğinize emin misiniz?'
        : 'Atamayı iptal etmek istediğinden emin misin?';
      if (!confirm(msg)) return;
    }
    try {
   ...
   * `band`: { startMin, endMin } verilirse yalnızca bu dakika dilimiyle kesişen atamalar çizilir (slot 18:30 olsa bile). */
  function matrisSlotHucre(sv, s, gunTarihi = tarih, matrisBandKey = '', band = null) {
    ...
    const bs = band?.startMin;
    const be = band?.endMin;
    const bantVar = band != null && Number.isFinite(bs) && Number.isFinite(be);
    const atamalarBuBantta = bantVar
      ? sv.atamalar.filter((a) => atamaBantIleKesisir(a, bs, be))
      : sv.atamalar;
    const ozetBuBantta = bantVar
      ? atamaListesiOzetiBandKesisim(sv.atamalar, bs, be)
      : atamaListesiIsOzeti(sv.atamalar);
    const slotGunEtik = slotAktifGunlerEtiket(sv.slot.aktif_gunler);
    ...
    return (
      ...
      >
        {/* Bant boşsa minimal hücre — sadece "Boş + drop zone". Slot detayları gizli, böylece kullanıcı buraya başka personel sürükleyebilir. */}
        {bantVar && sv.atamalar.length > 0 && atamalarBuBantta.length === 0 ? (
          <div style={{
            fontSize: 11,
            color: 'var(--text3)',
            fontStyle: 'italic',
            ...
            <div style={{ fontSize: 10, marginBottom: 4 }}>
              {fmtSaat(sv.slot.baslangic_saat)}–{fmtSaat(sv.slot.bitis_saat)} · {sv.slot.ad}
            </div>
            <div>Boş — başka personel sürükleyebilirsiniz</div>
        ...
        {sv.atamalar.length > 0 ? (() => {
          const oz = ozetBuBantta;
          if (!oz) return null;
          return (
            <div style={{ fontSize: 9, color: 'var(--text3)', marginBottom: 4, lineHeight: 1.3 }}>
              Atama özeti{bantVar ? ' (bu dilim)' : ''}: <span style={{ fontFamily: 'var(--font-mono, monospace)', fontWeight: 600, color: 'var(--text2)' }}>{oz.basStr}–{oz.bitStr}</span>
            </div>
          );
        })() : null}
        ...
        {slotGunEtik ? (
          <div style={{ fontSize: 9, color: 'var(--text3)', marginBottom: 6, lineHeight: 1.3 }}>
            Bu hafta: <strong style={{ color: 'var(--text2)', fontWeight: 600 }}>{slotGunEtik}</strong>
          </div>
        ) : null}
        ...
        ) : atamalarBuBantta.length === 0 ? (
          <div style={{ fontSize: 10, color: 'var(--text3)', fontStyle: 'italic', padding: '10px 4px', textAlign: 'center', borderTop: '1px dashed var(--border)', lineHeight: 1.35 }}>
            Bu saat diliminde mesai yok — çalışma saati başka aralıkta.
          </div>
        ) : (
          <div className="slot-avatar-stack" aria-label="Atanan personel">
            {atamalarBuBantta.map((a, i) => {
              ...
              const kesinKayit = String(a.durum || '') === 'onayli';
              const suruklenebilirChip = slottanSuruklenebilirMi(pChip, a);
              const tamAd = `${a.personel_ad || ''} ${a.personel_soyad || ''}`.trim() || 'Personel';
              const initials = avatarBasHarfler(a.personel_ad, a.personel_soyad);
              const chipTitle = kesinKayit
                ? `${tamAd} · ${s.sube_ad || 'Şube'} — kaydedilmiş atama (kilitli); taşımak için önce iptal edin · ✕ iptal`
                : `${tamAd} · ${s.sube_ad || 'Şube'}${a.yemek_sube_ad ? ` · 🍽 ${a.yemek_sube_ad}` : ''} — sürükleyerek taşı · ✕ tek atama iptal`;
              return (
                <div
                  key={a.id}
                ...
                >
                  <div
                    ...
                    draggable={suruklenebilirChip}
                    onDragStart={(e) => {
                      e.stopPropagation();
                      if (!suruklenebilirChip) {
                        e.preventDefault();
                        return;
                      }
                    ...
                    style={{
                      ...
                      border: kesinKayit ? '2px solid rgba(234,179,8,0.95)' : '2px solid var(--bg2)',
                      boxShadow: kesinKayit
                        ? '0 0 0 2px rgba(250,204,21,0.35), 0 2px 8px rgba(0,0,0,0.2)'
                      ...
                      cursor: suruklenebilirChip ? 'grab' : 'not-allowed',
                      opacity: kesinKayit ? 1 : undefined,
                    }}
                  >
                    {initials}
                  </div>
                  ...
                  <button
                    ...
                    onClick={(e) => { e.stopPropagation(); atamaIptal(a.id, false, a.durum); }}
                    title={kesinKayit ? `${tamAd} — kilitli atamayı iptal et` : `${tamAd} atamasını iptal et`}
                  >
                    ×
                  </button>
                </div>
        ...
        {/* HÜCRE SAAT LİSTESİ — kim hangi saatte (slot dışı uzayanları da göster); bant görünümünde yalnız bu dilime düşenler */}
        {atamalarBuBantta.length > 0 && (
          <div style={{ marginTop: 6, paddingTop: 4, borderTop: '1px dashed var(--border)', display: 'flex', flexDirection: 'column', gap: 2 }}>
            {atamalarBuBantta.map((a) => {
        ...
        {atamalarBuBantta.length > 0 ? (() => {
          const ads = [...new Set(atamalarBuBantta.map((at) => at.yemek_sube_ad).filter(Boolean))];
  ...
  return (
    <div className="page" style={{ paddingBottom: altPanelAcik ? 240 : 64 }}>
      ...
      <div className="card mb-14" style={{ padding: 0, overflow: 'hidden', border: '1px solid var(--border)' }}>
        ...
        {izinHaftaOzet && (
          <div
            ...
            style={{
              padding: '10px 16px',
              background: (izinHaftaOzet.izin_gormeyen_sayisi || 0) > 0
          ...
          >
            {(izinHaftaOzet.izin_gormeyen_sayisi || 0) > 0 ? (
              <>
              ...
              <>
                <strong style={{ color: '#15803d' }}>Yasal izin durumu</strong>
                {' '}(hafta {izinHaftaOzet.hafta_pazartesi} – {izinHaftaOzet.hafta_pazar}):
                {' '}<strong>bütün aktif personeller izin kullanmıştır</strong>.
              </>
            )}
          </div>
        )}
        ...
        <div style={{ padding: '12px 16px', borderBottom: '1px solid var(--border)', background: 'rgba(79,142,247,0.07)' }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginTop: 10, alignItems: 'center' }}>
            ...
            <button type="button" className="btn btn-sm btn-secondary" title="Tüm şubelerde slot üret" onClick={() => slotUretTumSubelerAc()}>⚙ Tüm şubelerde slot</button>
            <button
              type="button"
        ...
        <div className="card" style={{ padding: 12, position: 'sticky', top: 10, maxHeight: '85vh', overflowY: 'auto' }}>
          ...
          {havuzKaynakTarih !== tarih && havuzYukleniyor && !havuzGunPlani ? (
            ...
            filtrelenmisHavuz.map((p) => {
              ...
              const atanmisVurgulu = atanmisBugun && g.seviye === 'ok';
              const kartKenar = atanmisVurgulu
                ? '2px solid rgba(34,197,94,0.72)'
                : g.border;
              const kartGolge = atanmisVurgulu
                ? '0 0 0 4px rgba(34,197,94,0.14), 0 10px 26px rgba(34,197,94,0.14)'
                : g.seviye === 'kritik' ? '0 0 0 1px rgba(239,68,68,0.2)'
              ...
              const solSeritGen = atanmisVurgulu ? 7 : 5;
                  ...
                  style={{
                    ...
                    background: g.bg && g.bg !== 'transparent' ? g.bg
                              : (izinli ? 'rgba(239,68,68,0.11)'
                                : atanmisBugun ? 'rgba(34,197,94,0.14)'
                                  : planYok ? 'rgba(245,158,11,0.06)' : 'var(--bg3)'),
                ...
                >
                  ...
                  <div style={{ flex: 1, padding: '8px 10px 8px 8px', minWidth: 0 }}>
                    {atanmisBugun && (
                      <div
                        style={{
                          ...
                          marginBottom: 10,
                          padding: '7px 10px',
                          background: 'linear-gradient(90deg, rgba(34,197,94,0.42), rgba(79,142,247,0.12))',
                      ...
                      >
                        <span style={{ fontSize: 10, fontWeight: 900, letterSpacing: '0.06em', color: '#14532d' }}>
                          ✓ BU GÜNE ATANMIŞ
                        </span>
                        <span style={{ fontSize: 10, fontWeight: 800, fontFamily: 'var(--font-mono, monospace)', color: '#166534', flexShrink: 0 }}>
                          {atanmisSayisi > 0 ? `${atanmisSayisi} görev` : 'planlı'}
                        </span>
                      </div>
                    )}
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 6 }}>
                      ...
                      <span style={{
                        fontSize: calisiyor ? 10 : 9,
                        fontWeight: 900,
                        letterSpacing: calisiyor ? 0.12 : 0.3,
                        padding: calisiyor ? '4px 9px' : '2px 6px',
                        borderRadius: calisiyor ? 999 : 4,
                        flexShrink: 0,
                        background: izinli ? 'rgba(239,68,68,0.12)' : calisiyor ? 'rgba(34,197,94,0.35)' : planYok ? '#ffedd5' : '#f1f5f9',
                        color: izinli ? '#b91c1c' : calisiyor ? '#14532d' : planYok ? '#c2410c' : '#475569',
                        border: calisiyor ? '2px solid rgba(34,197,94,0.65)' : undefined,
                        boxShadow: calisiyor ? '0 1px 0 rgba(255,255,255,0.35) inset' : undefined,
                      }}>
        ...
        <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginLeft: 'auto', alignItems: 'center' }}>
              ...
              {gorunumModu === 'personel_hafta' && personelHafta && (
                <>
                  ...
                  <button type="button" className="btn btn-sm btn-primary" onClick={() => mesaiHesaplaPanel()}>🧮 Mesai Hesapla</button>
                </>
              )}
            </div>
            {gorunumModu === 'gun_matris' && (
          ...
          {gorunumModu === 'gun_gantt' ? (
                            ...
                            return (
                              ...
                              >
                                {hucre.length === 0 ? (
                                  ...
                                  hucre.map(({ sv, s: subeRow, bandKey }) => matrisSlotHucre(sv, subeRow, tarih, bandKey, band))
                                )}
                              </div>
                            );
                          })}
                              ...
                              return (
                                ...
                                >
                                  {!sub ? (
                                    ...
                                    hucre.map(({ sv, s: subeRow, bandKey }) => matrisSlotHucre(sv, subeRow, g.iso, bandKey, band))
                                  )}
                                </div>
                              );
                            })}
            ...
            <div style={{ padding: 12, overflowX: 'auto' }}>
              {mesaiHesap && mesaiHesap.pzt === (personelHafta?.pazartesi || pazartesiSecili) && (
                <div style={{ marginBottom: 12, border: '1px solid var(--border)', borderRadius: 10, background: 'var(--bg2)', padding: 10 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 10, marginBottom: 8 }}>
                    <div style={{ fontSize: 12, fontWeight: 800 }}>
                      Mesai Hesaplama Alanı · hafta {mesaiHesap.pzt}
                    </div>
                    <button type="button" className="btn btn-sm btn-secondary" onClick={() => setMesaiHesap(null)}>Kapat</button>
                  </div>
                  <div style={{ fontSize: 11, color: 'var(--text3)', marginBottom: 8 }}>
                    Kural: her gün için <strong>atanan saat - tanımlı günlük mesai</strong>; hafta sonunda <strong>atanan toplam - tanımlı haftalık mesai</strong>.
                  </div>
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: 8 }}>
                    {(mesaiHesap.satirlar || []).map((m) => (
                      <div key={`mh-${m.personel_id}`} style={{ border: '1px solid var(--border)', borderRadius: 8, background: 'var(--bg)', padding: 8 }}>
                        <div style={{ fontWeight: 800, fontSize: 12 }}>{m.adSoyad}</div>
                        <div style={{ fontSize: 10, color: 'var(--text3)', marginBottom: 6 }}>{m.sube}</div>
                        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', fontSize: 11, marginBottom: 6 }}>
                          <span>Tanımlı gün: <strong>{m.gunlukTanimli.toFixed(1)}s</strong></span>
                          <span>Tanımlı hafta: <strong>{m.haftalikTanimli.toFixed(1)}s</strong></span>
                          <span>Atanan hafta: <strong>{m.haftalikAtanan.toFixed(1)}s</strong></span>
                          <span style={{ color: m.haftalikFark > 0 ? '#dc2626' : '#16a34a', fontWeight: 700 }}>
                            Haftalık fark: {m.haftalikFark >= 0 ? '+' : ''}{m.haftalikFark.toFixed(1)}s
                          </span>
                        </div>
                        <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
                          {m.gunDetay.map((g) => (
                            <span
                              key={`${m.personel_id}-${g.iso}`}
                              title={`${g.iso} · atanan ${g.atanan.toFixed(1)}s · fark ${g.fark >= 0 ? '+' : ''}${g.fark.toFixed(1)}s`}
                              style={{
                                fontSize: 10,
                                border: '1px solid var(--border)',
                                borderRadius: 999,
                                padding: '2px 6px',
                                background: g.izinli ? 'rgba(239,68,68,0.10)' : g.fark > 0 ? 'rgba(239,68,68,0.10)' : 'rgba(34,197,94,0.10)',
                                color: g.izinli ? '#b91c1c' : g.fark > 0 ? '#b91c1c' : '#166534',
                                fontWeight: 700,
                              }}
                            >
                              {personelHaftaGunBaslik(g.iso)} {g.izinli ? 'İZN' : `${g.fark >= 0 ? '+' : ''}${g.fark.toFixed(1)}s`}
                            </span>
                          ))}
                        </div>
                      </div>
      ...
      {slotUretGunModal && (
        <SlotUretGunSecimModal
          key={slotUretGunModal.mode === 'tek' ? `tek-${slotUretGunModal.subeId}` : 'tum-subeler'}
          baslik={slotUretGunModal.mode === 'tek'
            ? `${slotUretGunModal.subeAd || 'Şube'} — şube saatlerinden slot (gün seçimi)`
            : 'Tüm şubeler — şube saatlerinden slot (gün seçimi)'}
          onClose={() => setSlotUretGunModal(null)}
          onUret={slotUretGunOnayla}
        />
      )}
      {dropSaatModal && (
        <DropAtamaSaatModal
...
// ═══════════════════════════════════════════════════════════════════
// AUTO SLOT ÜRET — hangi günlerde geçerli (API `aktif_gunler`)
// ═══════════════════════════════════════════════════════════════════
function SlotUretGunSecimModal({ baslik, onClose, onUret }) {
  const [gunler, setGunler] = useState(() => [1, 2, 3, 4, 5, 6, 7]);
  const [busy, setBusy] = useState(false);

  function gunToggle(g) {
    const yeni = gunler.includes(g)
      ? gunler.filter((x) => x !== g)
      : [...gunler, g].sort((a, b) => a - b);
    setGunler(yeni);
  }

  function preset(tip) {
    if (tip === 'all') setGunler([1, 2, 3, 4, 5, 6, 7]);
    else if (tip === 'ici') setGunler([1, 2, 3, 4, 5]);
    else if (tip === 'son') setGunler([6, 7]);
    else if (tip === 'pt') setGunler([1]);
    else if (tip === 'cum') setGunler([5]);
  }

  async function devam() {
    const g = normalizeAktifGunler(gunler);
    if (!g.length) {
      window.alert('En az bir gün seçin.');
      return;
    }
    setBusy(true);
    try {
  ...
  return (
    <Modal onClose={onClose} title={baslik}>
      <p style={{ fontSize: 12, color: 'var(--text3)', marginTop: 0, lineHeight: 1.45 }}>
        Üretilen <strong>AUTO:</strong> slotlar yalnızca seçtiğiniz günlerde planda görünür (ör. sadece hafta içi veya tek gün).
      </p>
      <div style={{ marginBottom: 10 }}>
        <span style={{ fontSize: 11, fontWeight: 700, color: 'var(--text3)' }}>Hızlı seçim</span>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: 6 }}>
          <button type="button" className="btn btn-sm btn-secondary" onClick={() => preset('all')}>Tüm hafta</button>
          <button type="button" className="btn btn-sm btn-secondary" onClick={() => preset('ici')}>Hafta içi</button>
          <button type="button" className="btn btn-sm btn-secondary" onClick={() => preset('son')}>Cmt–Paz</button>
          <button type="button" className="btn btn-sm btn-secondary" onClick={() => preset('pt')}>Sadece Pzt</button>
          <button type="button" className="btn btn-sm btn-secondary" onClick={() => preset('cum')}>Sadece Cum</button>
        </div>
      </div>
      <div className="form-group" style={{ marginBottom: 12 }}>
        <label style={{ fontWeight: 700 }}>Günler</label>
        <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap', marginTop: 6 }}>
          {TR_GUNLER.map((name, i) => {
            const idx = i + 1;
            const aktif = gunler.includes(idx);
            return (
              <button
                key={idx}
                type="button"
      ...
      <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, marginTop: 8 }}>
        ...
        <button type="button" className="btn btn-primary" onClick={devam} disabled={busy}>{busy ? '…' : 'Devam'}</button>
      </div>
    </Modal>
  );
}
...
function SlotModal({
  ...
  const [form, setForm] = useState(() => {
    if (slot) {
      return {
        ...slot,
        sube_id,
        aktif_gunler: normalizeAktifGunler(slot.aktif_gunler),
      };
    }
    return {
      sube_id,
  ...
  async function kaydet() {
    const ag = normalizeAktifGunler(form.aktif_gunler);
    if (!ag.length) {
      window.alert('En az bir hafta günü seçmelisiniz.');
      return;
    }
    setBusy(true);
    try {
      const body = { ...form, sube_id, aktif_gunler: ag };
      if (slot?.id) {
  ...
  return (
    <Modal onClose={onClose} title={slot?.id ? 'Slot Düzenle' : 'Yeni Slot'}>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
        ...
        <div className="form-group" style={{ gridColumn: '1/-1', padding: '10px 12px', background: 'var(--bg3)', borderRadius: 8, border: '1px solid var(--border)' }}>
          <label style={{ fontWeight: 800 }}>Hangi günlerde geçerli?</label>
          <p style={{ fontSize: 11, color: 'var(--text3)', margin: '6px 0 8px', lineHeight: 1.35 }}>
            Bu slot yalnızca işaretlediğiniz günlerin vardiya planında görünür; yalnızca belirli bir gün için de kısıtlayabilirsiniz.
          </p>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 8 }}>
            <button type="button" className="btn btn-sm btn-secondary" onClick={() => setForm({ ...form, aktif_gunler: [1, 2, 3, 4, 5, 6, 7] })}>Tüm hafta</button>
            <button type="button" className="btn btn-sm btn-secondary" onClick={() => setForm({ ...form, aktif_gunler: [1, 2, 3, 4, 5] })}>Hafta içi</button>
            <button type="button" className="btn btn-sm btn-secondary" onClick={() => setForm({ ...form, aktif_gunler: [6, 7] })}>Cmt–Paz</button>
          </div>
          <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
            {TR_GUNLER.map((g, i) => {
              const idx = i + 1;
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
        ...
        <div className="form-group">
          ...
          <div style={{ fontSize: 11, color: 'var(--text3)', marginTop: 6, lineHeight: 1.45 }}>
            İşletme kuralı: <strong>TAM</strong> personelde tipik üst sınır <strong>57 saat</strong>,{' '}
            <strong>PART</strong> için <strong>33 saat</strong>. Bu üzerindeki planlanan süreler sistemde{' '}
            <strong>ek mesai</strong> uyarısı olarak görünür (personel kartındaki çalışma türüne göre varsayılan önerilir).
          </div>
        </div>
        <div className="form-group">
    ...
    <Modal onClose={onClose} title="Atama saati">
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: 6, alignItems: 'center' }}>
        <button
          ...
          className="btn btn-sm btn-secondary"
          disabled={busy}
          title="Slot çerçevesinin tamamını yazar; personel PART vb. ise genelde uygun olan sunucudan gelen öneriyi kullanın."
          onClick={() => { setBas(slotBasDef); setBit(slotBitDef); }}
        >
          Slot saati ({slotBasDef}–{slotBitDef})
        </button>
        {/* SLOT BÖLME — uzun slotlar için ilk yarı / ikinci yarı / kalan boşluk */}
        {(() => {
          if (!slotBasDef || !slotBitDef) return null;
          const toMin = (s) => { const [h,m]=s.split(':').map(Number); return (h||0)*60+(m||0); };
          const toStr = (m) => `${String(Math.floor(m/60)).padStart(2,'0')}:${String(m%60).padStart(2,'0')}`;
          const sb = toMin(slotBasDef);
          const se = toMin(slotBitDef);
          const sureSlot = se - sb;
          if (sureSlot < 240) return null; // 4 saatten kısa slotlarda gereksiz
          const ortaMin = sb + Math.round(sureSlot / 2 / 15) * 15; // 15 dk kademe
          const orta = toStr(ortaMin);
          // Mevcut atamalar (boşluk hesabı için)
          const mevcutAt = (slotRow?.sv?.atamalar || []).map(a => ({
            bas: toMin(fmtSaat(a.baslangic_saat) || slotBasDef),
            bit: toMin(fmtSaat(a.bitis_saat) || slotBitDef),
          })).sort((a,b) => a.bas - b.bas);
          // İlk boşluk: slot başından ilk atamaya kadar VEYA atamadan sonra slot sonuna kadar
          let bosBas = null, bosBit = null;
          if (mevcutAt.length === 0) {
            // boşluk yok — sadece bölme butonları yeter
          } else {
            // Slot içinde kapsanmamış ilk aralığı bul
            let pivot = sb;
            for (const a of mevcutAt) {
              if (a.bas > pivot) {
                bosBas = pivot; bosBit = a.bas; break;
              }
              pivot = Math.max(pivot, a.bit);
            }
            if (bosBas === null && pivot < se) {
              bosBas = pivot; bosBit = se;
            }
          }
          return (
            <>
              <button type="button" className="btn btn-sm btn-secondary" disabled={busy}
                title={`Slotun ilk yarısı (${slotBasDef}–${orta})`}
                onClick={() => { setBas(slotBasDef); setBit(orta); }}>
                🪓 İlk yarı ({slotBasDef}–{orta})
              </button>
              <button type="button" className="btn btn-sm btn-secondary" disabled={busy}
                title={`Slotun ikinci yarısı (${orta}–${slotBitDef})`}
                onClick={() => { setBas(orta); setBit(slotBitDef); }}>
                🪓 İkinci yarı ({orta}–{slotBitDef})
              </button>
              {bosBas !== null && bosBit !== null && (bosBit - bosBas) >= 60 && (
                <button type="button" className="btn btn-sm btn-primary" disabled={busy}
                  title={`Bu slotta mevcut atamaların bıraktığı boşluk: ${toStr(bosBas)}–${toStr(bosBit)}`}
                  onClick={() => { setBas(toStr(bosBas)); setBit(toStr(bosBit)); }}>
                  🎯 Kalan boşluk ({toStr(bosBas)}–{toStr(bosBit)})
                </button>
              )}
            </>
          );
      ...
      <p style={{ fontSize: 11, color: 'var(--text3)', marginTop: 14, marginBottom: 0, lineHeight: 1.45 }}>
        <strong>Kaydet</strong> ile vardiya saati kesinlenir; kayıt <strong>kilitlenir</strong> (taşıyamazsınız). Taşımak veya süreyi yeniden seçmek için önce slotta ✕ ile iptal edin.
      </p>
      <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, marginTop: 14 }}>
        <button type="button" className="btn btn-secondary" onClick={onClose} disabled={busy}>İptal</button>
        <button type="button" className="btn btn-primary" onClick={handleTamam} disabled={busy}>{busy ? '…' : 'Kaydet'}</button>
      </div>
    </Modal>
  );
}
