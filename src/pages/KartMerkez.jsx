...
export default function KartMerkez({ onNavigate }) {
  ...
  async function stratejiYukle() {
    ...
    try {
      ...
      console.error(e);
      // Strateji yüklenemedi — kullanıcıya sessizce geçme
        ...
        <div>
          ...
          {strateji?.hata && (
            <div style={{padding:'10px 14px',background:'rgba(220,50,50,0.07)',
              borderRadius:8,border:'1px solid var(--red)',marginBottom:12,fontSize:12,color:'var(--red)'}}>
              ⚠️ Strateji motoru yüklenemedi: {strateji.hata}
            </div>
          )}
          {strateji && !strateji.hata && (
            <div>
