...
export default function TvMenuYonetim() {
  ...
  const tipSet = (i, ek, tip) => setKurgu(k => k.map((s, j) => j !== i ? s : { ...s, [ek]:
    ...
    : tip === 'oneri' ? { oneri: true }
    : tip === 'film' ? { film: true }
    : tip === 'carousel' ? { carousel: true }
    : { v: klipler[0] || '', k: '', b: '' } }));
              ...
              <div key={i} style={{ border: '1px solid var(--border)', borderRadius: 8, padding: 10, marginBottom: 8, background: 'var(--bg2, var(--bg))' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(190px, 1fr))', gap: 8 }}>
                  {[['e1', '1 · Menü/karar'], ['e2', '2 · Deneyim'], ['e3', '3 · İmza']].map(([ek, lbl]) => {
                    const ic = s[ek] || {}; const tip = ic.spot ? 'spot' : ic.cok ? 'cok' : ic.yeni ? 'yeni' : ic.sezon ? 'sezon' : ic.hero ? 'hero' : ic.gunun ? 'gunun' : ic.vitrin ? 'vitrin' : ic.eslesme ? 'eslesme' : ic.oneri ? 'oneri' : ic.film ? 'film' : ic.carousel ? 'carousel' : (ic.menu != null ? 'menu' : 'video');
                    const OTO = { spot: 'İmza ürün + fiyat + “yanına yakışır” otomatik. İmza/pair’i Yaşayan Menü Ayarları’ndan seç.', cok: '🏆 Bu hafta en çok satılan 3 ürün — canlı satış verisinden otomatik.', yeni: '✨ Menüye yeni eklenen ürünler — canlı veriden otomatik.', sezon: '☀️ Mevsim/saate göre öneri (yazın buzlu, kışın sıcak) — canlı, fiyatsız fısıltı.', hero: '🥤 Haftanın en çok seçileni — ürünün KENDİ videosu tam ekran, FİYATSIZ (vaat sahnesi).', gunun: '⭐ Günün seçimi — nötr mekân zemini + bardak + fiyat İLK KEZ + “yanına yakışır” (satış zirvesi).', vitrin: '🏪 Üçlü vitrin — en çok satan 3 ürün yan yana, bardak + 8oz/14oz/Buzlu fiyat blokları + extralar şeridi.', eslesme: '🍽️ Saat eşleştirmesi — sabah kahve+kruvasan, öğle serin+cookie, akşam imza+tatlı (otomatik).', oneri: '🤫 Baristanın önerisi — az satılan ürünü "bilenin seçimi" olarak öne çıkarır (buğulu zemin, bardak solda).', film: '🎬 Ürün vitrin filmi — gerçek ürün fotoğrafı fırlar+yüzer, arkada gerçek sıçrama akar (en çok satan, canlı).', carousel: '🎠 Ürün carousel — en çok satan 3 ürün sırayla film dilinde döner (kahve→video, milkshake→foto). Uzun slot (~13sn) öner.' };
                    return (
                      <div key={ek} style={{ border: '1px solid var(--border)', borderRadius: 6, padding: 8 }}>
                        ...
                        <select value={tip} onChange={e => tipSet(i, ek, e.target.value)} style={{ width: '100%', fontSize: 11, marginBottom: 4 }}>
                          ...
                          <option value="hero">🥤 Kahraman ürün</option><option value="gunun">⭐ Günün seçimi</option><option value="vitrin">🏪 Üçlü vitrin</option><option value="eslesme">🍽️ Saat eşleştirmesi</option><option value="oneri">🤫 Baristanın önerisi</option><option value="film">🎬 Ürün vitrin filmi</option><option value="carousel">🎠 Ürün carousel (3'lü)</option>
                        </select>
                        {tip === 'menu'
                          ? <>
