...
export default function Personel() {
  ...
  const [vardiyaAktarPid, setVardiyaAktarPid] = useState(null);
  ...
  async function vardiyadanDoldur(pid) {
    setVardiyaAktarPid(pid);
    try {
      const res = await api(`/personel-aylik/${pid}/vardiya-aktar?yil=${aylikYil}&ay=${aylikAy}`, { method: 'POST' });
      toast(`Vardiya aktarıldı — Net: ${parseInt(res.hesaplanan_net).toLocaleString('tr-TR')} ₺`);
      publishGlobalDataRefresh('personel-maas-vardiya-aktar');
      loadAylik();
    } catch (e) {
      toast(e.message, 'red');
    } finally {
      setVardiyaAktarPid(null);
    }
  }
              ...
              {aylikData.personeller.map(p => {
                ...
                return (
                  <div key={p.personel_id} style={{
                    ...
                    <div style={{
                      fontSize:11,color:'var(--text3)',marginBottom:10,padding:'8px 10px',
                      ...
                      {!onaylandi && (
                        <button
                          ...
                          disabled={vardiyaAktarPid === p.personel_id}
                          onClick={() => vardiyadanDoldur(p.personel_id)}
                        >
                          {vardiyaAktarPid === p.personel_id ? '…' : 'Vardiyadan doldur'}
                        </button>
                      )}
