        ...
        <div className="card">
          ...
          {tumUyarilar.length === 0 ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8, maxHeight: 320, overflowY: 'auto' }}>
              {tumUyarilar.map((u, i) => {
                if (u.kural === 6) {
                  const renk = u.gun_kalan === 0 ? 'var(--red)' : '#f07040';
                  return (
                    <div key={i} style={{
                      padding: '10px 12px', borderRadius: 6,
                      background: u.gun_kalan === 0 ? 'rgba(220,50,50,0.06)' : 'rgba(240,112,64,0.06)',
                      borderLeft: `3px solid ${renk}`,
                    }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 8 }}>
                        <div style={{ flex: 1 }}>
                          <div style={{ fontSize: 12, fontWeight: 600 }}>🧾 {u.baslik}</div>
                          <div style={{ fontSize: 11, color: 'var(--text3)', marginTop: 2 }}>
                            {u.vade_tarihi} · {u.gun_kalan === 0 ? '🔴 BUGÜN' : u.gun_kalan === 1 ? '⚠️ Yarın son gün' : `${u.gun_kalan} gün kaldı`}
                          </div>
                        </div>
                        <div style={{ textAlign: 'right', flexShrink: 0 }}>
                          <div style={{ fontSize: 14, fontWeight: 700, fontFamily: 'var(--font-mono)', color: renk }}>{fmt(u.tutar)}</div>
                        </div>
                      </div>
                    </div>
                  );
