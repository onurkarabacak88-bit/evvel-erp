...
export default function Panel({ onNavigate }) {
  ...
  const [onayModal, setOnayModal] = useState(null);   // {id, aciklama, tutar, islem_turu}
  const [onayTutar, setOnayTutar] = useState('');
  ...
  async function odemeOnayla(odemeId, tutar) {
    ...
    try {
      ...
      toast('🔔 Onay kuyruğuna alındı — Onay Merkezi'nden onaylayın', 'yellow');
  ...
  function onayKuyrukOnayla(o) {
    // Modal aç — tutar girilebilsin
    setOnayModal(o);
    setOnayTutar('');
  }

  async function onayKuyrukOnaylaGonder() {
    if (!onayModal || loadingBtn) return;
    setLoadingBtn(true);
    try {
      const tutar = onayTutar ? parseFloat(onayTutar) : null;
      const params = tutar ? `?tutar=${tutar}` : '';
      await api(`/onay-kuyrugu/${onayModal.id}/onayla${params}`, { method: 'POST' });
      const gosterilen = tutar || parseFloat(onayModal.tutar);
      toast(`✓ Onaylandı (${gosterilen.toLocaleString('tr-TR')} ₺) — kasadan düşüldü`);
      setOnayModal(null);
      load();
    } catch (e) { toast(e.message, 'red'); }
    finally { setLoadingBtn(false); }
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          {(anomali?.sorunlu > 0 || anomali?.alarm_var) && (
            <span className="badge badge-red" style={{ cursor: 'pointer' }} onClick={() => nav('ledger')}
              ...
              ⚠️ {anomali?.sorunlu > 0
                ? `${anomali.sorunlu} kasa anomalisi`
                : anomali?.alarmlar?.[0]?.mesaj || 'Kasa uyarısı'}
            </span>
          )}
            ...
            return (
              <div key={i} style={{
                ...
                <button className="btn btn-sm" style={{
                  ...
                  setKiraForm({
                    tutar: u.tutar || '',
                    // SOZLESME_UZAT: min tarih = sözleşme bitiş tarihi (öncesi seçilemesin)
                    gecerlilik_tarihi: u.aksiyon === 'SOZLESME_UZAT' && u.tarih
                      ? u.tarih   // bitiş tarihi — başlangıç noktası
                      : new Date().toISOString().split('T')[0],
                    kira_artis_periyot: '',
                    sozlesme_sure_ay: '',
                    _min_tarih: u.aksiyon === 'SOZLESME_UZAT' ? u.tarih : null,
                  });
                }}>
                  {btnLabel}
                </button>
                ...
                <div className="form-group">
                  ...
                  <span style={{fontSize:11,color:'var(--text3)'}}>
                    {kiraModal.aksiyon==='SOZLESME_UZAT'
                      ? `Mevcut sözleşme bitiş: ${kiraModal.tarih} — bu tarihten önce olamaz`
                      : `Mevcut artış tarihi: ${kiraModal.tarih} — bu tarihten önce olamaz`}
                  </span>
                  {kiraModal.aksiyon==='TUTAR_GUNCELLE' && (
              ...
              <button className="btn btn-primary"
                ...
                onClick={async()=>{
                  ...
                  try {
                    ...
                    toast('🔔 Güncelleme onay kuyruğuna alındı — Onay Merkezi'nden onaylayın', 'yellow');
                    setKiraModal(null);
                    load(); // Panel verilerini yenile — onay sayacı güncellensin
                  } catch(e) {
                    toast(e.message, 'red');
                  } finally {
            ...
            {onaylar.map(o => (
              <div key={o.id} style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  <button className="btn btn-primary btn-sm" onClick={() => onayKuyrukOnayla(o)}>✓ Onayla</button>
        ...
        <div className="card" style={{ marginBottom: 16 }}>
          ...
          {(anomali?.sorunlu > 0 || anomali?.alarm_var) && (
            <div style={{
              ...
              {anomali?.sorunlu > 0 && (
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom: anomali?.alarmlar?.length ? 8 : 0 }}>
              ...
              {anomali?.alarmlar?.filter(a => a.seviye === 'KRITIK' || a.seviye === 'UYARI').map((a, i) => (
                <div key={i} style={{ fontSize: 12, color: a.seviye === 'KRITIK' ? 'var(--red)' : 'var(--yellow)',
                  fontWeight: 600, marginTop: 4 }}>
                  {a.seviye === 'KRITIK' ? '🚨' : '⚠️'} {a.mesaj}
                </div>
              ))}
            </div>
          )}
      ...
      {onayModal && (
        <div className="modal-overlay" onClick={e=>e.target===e.currentTarget&&setOnayModal(null)}>
          <div className="modal">
            <div className="modal-header">
              <h3>✅ İşlemi Onayla</h3>
              <button className="modal-close" onClick={()=>setOnayModal(null)}>✕</button>
            </div>
            <div className="modal-body">
              <div style={{background:'var(--bg3)',borderRadius:8,padding:'12px 14px',marginBottom:16}}>
                <div style={{fontSize:14,fontWeight:600,marginBottom:6}}>
                  {({'SABIT_GIDER':'🏠 Sabit Gider','KART_ODEME':'💳 Kart Ödemesi',
                     'PERSONEL_MAAS':'👥 Personel Maaşı','VADELI_ODEME':'📦 Vadeli Ödeme',
                     'ODEME_PLANI':'📋 Ödeme Planı'})[onayModal.islem_turu] || onayModal.islem_turu}
                </div>
                <div style={{fontSize:12,color:'var(--text3)'}}>{onayModal.aciklama}</div>
                <div style={{marginTop:8,fontSize:13}}>
                  Kayıtlı tutar: <strong style={{color:'var(--red)',fontFamily:'var(--font-mono)'}}>
                    {parseInt(onayModal.tutar).toLocaleString('tr-TR')} ₺
                  </strong>
                </div>
              </div>
              <div className="form-group">
                <label>Ödenen Tutar (₺) — boş bırakırsan kayıtlı tutar geçer</label>
                <input type="number" value={onayTutar}
                  onChange={e=>setOnayTutar(e.target.value)}
                  placeholder={`Kayıtlı: ${onayModal.tutar}`} autoFocus />
                {onayTutar && (
                  <span style={{fontSize:11,color:'var(--text3)',marginTop:4,display:'block'}}>
                    Kasadan düşülecek: <strong style={{color:'var(--red)'}}>
                      {parseFloat(onayTutar).toLocaleString('tr-TR')} ₺
                    </strong>
                    {kasa - parseFloat(onayTutar) < 0 && (
                      <span style={{marginLeft:8,color:'var(--red)'}}>⚠️ Kasa eksiye düşer!</span>
                    )}
                  </span>
                )}
              </div>
            ...
            <div className="modal-footer">
              <button className="btn btn-danger btn-sm" style={{marginRight:'auto'}}
                onClick={()=>{onayKuyrukReddet(onayModal.id);setOnayModal(null);}}>
                ✕ Reddet
              </button>
              <button className="btn btn-secondary" onClick={()=>setOnayModal(null)}>Vazgeç</button>
              <button className="btn btn-primary" disabled={loadingBtn}
                onClick={onayKuyrukOnaylaGonder}>
                {loadingBtn ? '⏳...' : `✅ Onayla${onayTutar ? ` (${parseFloat(onayTutar).toLocaleString('tr-TR')} ₺)` : ' (kayıtlı tutar)'}`}
              </button>
            </div>
          </div>
        </div>
