...
export default function Panel({ onNavigate }) {
  ...
  const load = () => {
    ...
    Promise.all([
      ...
      // Vadeli uyarılar uyari_motoru'ndan geliyor (u içinde)
      setVadeliUyarilar((u || []).filter(x => x.kural === 6));
      setLoading(false);
  ...
  return (
    <div className="page">
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          {(anomali?.sorunlu > 0 || anomali?.alarm_var) && (
            <span className="badge badge-red" style={{ cursor: 'pointer' }} onClick={() => nav('ledger')}
              ...
              ⚠️ {anomali?.sorunlu > 0 ? `${anomali.sorunlu} kasa anomalisi` : anomali?.alarmlar?.[0]?.mesaj || 'Kasa uyarısı'}
            </span>
          )}
      ...
      {vadeliUyarilar.length > 0 && (
        <div style={{display:'flex',flexDirection:'column',gap:8,marginBottom:16}}>
          {vadeliUyarilar.map((v,i)=>{
            ...
            const renk = kritik ? 'var(--red)' : '#f07040';
      ...
      {/* ACİL: Bugün/geçmiş — yanıp söner, tam açık */}
      {kritikler.length > 0 && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 12 }}>
          {kritikler.map((u, i) => (
            <div key={i} className="blink" style={{
              background: 'rgba(220,50,50,0.10)', border: '2px solid var(--red)',
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ fontSize: 11, color: 'var(--text3)', marginTop: 2 }}>
                  {fmtDate(u.tarih)} · {fmt(u.tutar)}
                  {u.gun_farki < 0 && <span style={{ color: 'var(--red)', marginLeft: 6 }}>{Math.abs(u.gun_farki)} gün gecikmiş!</span>}
                </div>
              </div>
              <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
                <button className="btn btn-primary btn-sm" disabled={loadingBtn} onClick={() => { setOdemeModal(u); setManuelTutar(''); }}>✓ Ödendi</button>
                <button className="btn btn-secondary btn-sm" disabled={loadingBtn} onClick={() => odemeErtele(u.odeme_id)}>⏳</button>
              </div>
            </div>
          ))}
        </div>
      ...
      {/* TICKER: 3 gün içindeki ödemeler soldan sağa kayar */}
      {diger.filter(u => u.gun_farki <= 3).length > 0 && (
        <div style={{
          overflow: 'hidden', marginBottom: 10,
          background: 'rgba(240,112,64,0.08)', border: '1px solid #f07040',
          ...
          <div style={{
            ...
            {[...diger.filter(u => u.gun_farki <= 3), ...diger.filter(u => u.gun_farki <= 3)].map((u, i) => (
              <span key={i} style={{ fontSize: 12, fontWeight: 600, color: '#f07040' }}>
                🟠 {u.aciklama} — {fmt(u.tutar)} — {u.gun_farki === 0 ? 'BUGÜN' : `${u.gun_farki} gün`}
              </span>
            ))}
          </div>
        </div>
      ...
      {/* YAKLAŞAN ÖDEMELER — tek satır başlık, tıklayınca accordion */}
      {tumUyarilar.length > 0 && (
        <div style={{ marginBottom: 12 }}>
          <div onClick={() => setYaklaşanAcik(v => !v)} style={{
            display: 'flex', justifyContent: 'space-between', alignItems: 'center',
            padding: '10px 14px', borderRadius: 8, cursor: 'pointer',
            background: 'var(--bg2)', border: '1px solid var(--border)',
          }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <span style={{ fontSize: 13, fontWeight: 700 }}>📅 Yaklaşan Ödemeler</span>
              <span style={{ fontSize: 11, color: 'var(--text3)' }}>{tumUyarilar.length} ödeme</span>
              {kritikler.length > 0 && <span style={{ fontSize: 11, fontWeight: 700, color: 'var(--red)' }}>{kritikler.length} acil</span>}
            </div>
            <span style={{ fontSize: 12, color: 'var(--text3)' }}>{yaklaşanAcik ? '▲ Kapat' : '▼ Göster'}</span>
          </div>
          {yaklaşanAcik && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginTop: 6, maxHeight: 400, overflowY: 'auto' }}>
              {tumUyarilar.map((u, i) => {
                const renk = u.seviye === 'KRITIK' ? 'var(--red)' : u.renk === 'TURUNCU' ? '#f07040' : 'var(--yellow)';
                const bg   = u.seviye === 'KRITIK' ? 'rgba(220,50,50,0.08)' : u.renk === 'TURUNCU' ? 'rgba(240,112,64,0.07)' : 'rgba(220,160,0,0.05)';
                const acik = u.seviye === 'KRITIK' || acikOdemeler.has(i);
                return (
                  <div key={i} className={u.blink ? 'blink' : ''} style={{
                    borderRadius: 6, border: `1px solid ${renk}`, background: bg, overflow: 'hidden',
                    ...
                    <div onClick={() => { if (u.seviye === 'KRITIK') return; setAcikOdemeler(prev => { const s = new Set(prev); s.has(i) ? s.delete(i) : s.add(i); return s; }); }}
                      style={{ padding: '8px 12px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderLeft: `3px solid ${renk}` }}>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontSize: 12, fontWeight: 600, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{u.aciklama}</div>
                    ...
                    {acik && (
                      <div style={{ padding: '8px 12px 10px', borderTop: `1px solid ${renk}22` }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
        ...
        {/* SOL: ÖDEMELER — üstteki accordion'a taşındı */}
        <div className="card">
          <h3 style={{ fontSize: 13, fontWeight: 600, marginBottom: 8 }}>📅 Bu Ay Ödemeleri</h3>
          {tumUyarilar.length === 0 ? (
            <div className="empty"><p>Yaklaşan ödeme yok</p></div>
          ) : (
            <div style={{ fontSize: 12, color: 'var(--text3)' }}>
              Toplam <strong>{tumUyarilar.length}</strong> ödeme ·{' '}
              {fmt(tumUyarilar.reduce((s, u) => s + (u.tutar || 0), 0))} ₺
              <br/>
              <span style={{ color: 'var(--text3)', fontSize: 11 }}>↑ Üstteki listeden yönetin</span>
            </div>
          )}
          {false && tumUyarilar.map((u, i) => (null
          ))}
        </div>
      ...
      {(gelir30 > 0 || gider30 > 0) && (
        <div className="card" style={{ marginBottom: 16 }}>
          ...
          {(anomali?.sorunlu > 0 || anomali?.alarm_var) && (
            <div style={{ marginTop: 10, padding: '10px 14px', background: 'rgba(220,50,50,0.1)', border: '1px solid var(--red)', borderRadius: 6 }}>
              {anomali?.sorunlu > 0 && (
