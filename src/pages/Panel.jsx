...
export default function Panel({ onNavigate }) {
  ...
  const [odemeOzet, setOdemeOzet] = useState(null);      // nakit/kart kırılım özeti
  const [odemeOzetModal, setOdemeOzetModal] = useState(null); // {tip:'anlik'|'sabit'|'vadeli'|'nakit'|'kart', liste:[]}
  ...
  const load = () => {
    ...
    Promise.all([
      ...
      api('/panel/odeme-yontemi-ozet').catch(() => null),
    ]).then(([p, u, o, a, sg, su, og, vo, oo]) => {
    ...
    <div className="page">
      ...
      {panel.bugun_odemeler?.length > 0 && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 8 }}>
          {panel.bugun_odemeler.map((u, i) => {
            ...
            const animClass = gecikme ? 'blink' : 'blink';
            return (
              <div key={i} className={animClass} style={{
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ fontWeight: 700, fontSize: 13, color: 'var(--red)' }}>
                    {gecikme ? `⛔ ${Math.abs(u.gun_farki)} GÜN GECİKMİŞ` : '🚨 BUGÜN SON GÜN'} — {u.aciklama}
                  </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 12, marginBottom: 16 }}>
        {[
          ...
          (() => {
            const anlikNakit = panel.bu_ay_anlik_nakit || 0;
            const anlikKart  = panel.bu_ay_anlik_kart  || 0;
            const anlikToplam = panel.bu_ay_anlik_gider || 0;
            const sub = anlikKart > 0
              ? `💵 ${fmt(anlikNakit)} nakit · 💳 ${fmt(anlikKart)} kart`
              : 'Anlık giderler toplamı';
            return {
              label: '💸 Bu Ay Anlık Gider', value: fmt(anlikToplam),
              sub, renk: anlikToplam > 0 ? 'var(--red)' : 'var(--text3)',
              page: 'anlik-gider',
              onDetail: () => setOdemeOzetModal({ tip: 'anlik', baslik: 'Anlık Gider Detayı', liste: odemeOzet?.anlik?.liste || [] })
            };
          })(),
          (() => {
            ...
            const subText = durdurulmus > 0
              ...
              ? `⚠️ ${geciken} gecikmiş`
              : (() => {
                  const kategoriler = [...new Set((sabitGiderOzet.odenenler||[]).map(g => g.kategori).filter(Boolean))];
                  return kategoriler.length > 0 ? kategoriler.join(' · ') : 'Tümü güncel ✓';
                })();
            const subRenk = durdurulmus > 0 ? 'var(--red)' : geciken > 0 ? 'var(--yellow)' : 'var(--text3)';
            const sgNakit = sabitGiderOzet.nakit_odenen || 0;
            const sgKart  = sabitGiderOzet.kart_odenen  || 0;
            const sgKirilim = sgKart > 0 ? ` · 💵 ${fmt(sgNakit)} · 💳 ${fmt(sgKart)}` : '';
            return {
              label: '🏠 Sabit Gider (Ödenen)', value: fmt(sabitGiderOzet.toplam_odenen || 0),
              sub: subText + sgKirilim, renk: subRenk, page: 'sabit-giderler',
              onDetail: () => setOdemeOzetModal({ tip: 'sabit', baslik: 'Sabit Gider Detayı', liste: odemeOzet?.sabit?.liste || [] })
            };
          })(),
          (() => {
            const geciken = vadeliOzet.geciken_adet || 0;
            const bekleyen = vadeliOzet.bekleyen_adet || 0;
            const subText = geciken > 0
              ? `⚠️ ${geciken} gecikmiş`
              : bekleyen > 0
              ? `${bekleyen} bekleyen`
              : 'Tümü güncel ✓';
            const subRenk = geciken > 0 ? 'var(--red)' : bekleyen > 0 ? 'var(--yellow)' : 'var(--text3)';
            const vNakit = vadeliOzet.nakit_odenen || 0;
            const vKart  = vadeliOzet.kart_odenen  || 0;
            const vKirilim = vKart > 0 ? ` · 💵 ${fmt(vNakit)} · 💳 ${fmt(vKart)}` : '';
            return {
              label: '🛒 Vadeli Borç (Ödenen)', value: fmt(vadeliOzet.toplam_odenen || 0),
              sub: subText + vKirilim, renk: subRenk, page: 'vadeli-alimlar',
              onDetail: () => setOdemeOzetModal({ tip: 'vadeli', baslik: 'Vadeli Alım Detayı',
                nakit_liste: odemeOzet?.vadeli?.nakit_liste || [],
                kart_liste: odemeOzet?.vadeli?.kart_liste || [] })
            };
          })(),
        ].map(({ label, value, sub, renk, page, overlay }) => (
          <div key={label} className="metric-card" style={{ borderTop: `3px solid ${renk}`, cursor: 'pointer' }}
            onClick={() => onDetail ? onDetail() : overlay ? gecmisAc(overlay.baslik, overlay.endpoint) : nav(page)}
        ...
        {/* ── NAKİT / KART ÖZET KARTLARI ── */}
        {odemeOzet && (odemeOzet.genel.nakit_toplam > 0 || odemeOzet.genel.kart_toplam > 0) && (
          <div style={{ gridColumn: '1 / -1', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            {/* Nakit Kullanımı */}
            <div className="card" style={{ borderTop: '3px solid var(--green)', cursor: 'pointer', padding: '14px 18px' }}
              onClick={() => setOdemeOzetModal({
                tip: 'nakit', baslik: '💰 Bu Ay Nakit Ödemeler',
                liste: [
                  ...(odemeOzet.anlik?.liste || []).filter(r => r.odeme_yontemi !== 'kart').map(r => ({...r, kaynak: 'Anlık Gider'})),
                  ...(odemeOzet.sabit?.liste || []).filter(r => r.odeme_yontemi !== 'kart').map(r => ({...r, kaynak: 'Sabit Gider'})),
                  ...(odemeOzet.vadeli?.nakit_liste || []).map(r => ({...r, kaynak: 'Vadeli Alım'})),
                ]
              })}>
              <div style={{ fontSize: 11, color: 'var(--text3)', fontWeight: 600, marginBottom: 6 }}>💰 Bu Ay Nakit Ödemeler</div>
              <div style={{ fontSize: 22, fontWeight: 700, color: 'var(--green)' }}>{fmt(odemeOzet.genel.nakit_toplam)}</div>
              <div style={{ marginTop: 8, display: 'flex', gap: 12, fontSize: 11, color: 'var(--text3)', flexWrap: 'wrap' }}>
                {odemeOzet.anlik.nakit > 0 && <span>Anlık: {fmt(odemeOzet.anlik.nakit)}</span>}
                {odemeOzet.sabit.nakit > 0 && <span>Sabit: {fmt(odemeOzet.sabit.nakit)}</span>}
                {odemeOzet.vadeli.nakit > 0 && <span>Vadeli: {fmt(odemeOzet.vadeli.nakit)}</span>}
              </div>
              <div style={{ marginTop: 6, fontSize: 10, color: 'var(--text3)' }}>Tıkla: tüm nakit işlemler →</div>
            </div>

            {/* Kart Kullanımı */}
            <div className="card" style={{ borderTop: '3px solid #4a9eff', cursor: 'pointer', padding: '14px 18px' }}
              onClick={() => setOdemeOzetModal({
                tip: 'kart', baslik: '💳 Bu Ay Kart Ödemeleri',
                liste: [
                  ...(odemeOzet.anlik?.liste || []).filter(r => r.odeme_yontemi === 'kart').map(r => ({...r, kaynak: 'Anlık Gider'})),
                  ...(odemeOzet.sabit?.liste || []).filter(r => r.odeme_yontemi === 'kart').map(r => ({...r, kaynak: 'Sabit Gider'})),
                  ...(odemeOzet.vadeli?.kart_liste || []).map(r => ({...r, kaynak: 'Vadeli Alım', tutar: r.tutar})),
                ]
              })}>
              <div style={{ fontSize: 11, color: 'var(--text3)', fontWeight: 600, marginBottom: 6 }}>💳 Bu Ay Kart Ödemeleri</div>
              <div style={{ fontSize: 22, fontWeight: 700, color: '#4a9eff' }}>{fmt(odemeOzet.genel.kart_toplam)}</div>
              <div style={{ marginTop: 8, display: 'flex', gap: 12, fontSize: 11, color: 'var(--text3)', flexWrap: 'wrap' }}>
                {odemeOzet.anlik.kart > 0 && <span>Anlık: {fmt(odemeOzet.anlik.kart)}</span>}
                {odemeOzet.sabit.kart > 0 && <span>Sabit: {fmt(odemeOzet.sabit.kart)}</span>}
                {odemeOzet.vadeli.kart > 0 && <span>Vadeli: {fmt(odemeOzet.vadeli.kart)}</span>}
              </div>
              <div style={{ marginTop: 6, fontSize: 10, color: 'var(--text3)' }}>Tıkla: tüm kart işlemler →</div>
            </div>
          </div>
        )}
      ...
      {/* ── ÖDEME YÖNTEMİ DETAY MODALI ── */}
      {odemeOzetModal && (
        <div className="modal-overlay" onClick={e => e.target === e.currentTarget && setOdemeOzetModal(null)}>
          <div className="modal" style={{ maxWidth: 560 }}>
            <div className="modal-header">
              <h3>{odemeOzetModal.baslik}</h3>
              <button className="modal-close" onClick={() => setOdemeOzetModal(null)}>✕</button>
            </div>
            <div className="modal-body" style={{ maxHeight: '65vh', overflowY: 'auto' }}>
              {/* Vadeli — nakit ve kart ayrı bölümler */}
              {odemeOzetModal.tip === 'vadeli' ? (
                <>
                  {(odemeOzetModal.nakit_liste || []).length > 0 && (
                    <>
                      <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--text3)', marginBottom: 8 }}>💵 NAKİT ÖDEMELER</div>
                      {odemeOzetModal.nakit_liste.map((r, i) => (
                        <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 10px', borderRadius: 6, background: 'var(--bg3)', marginBottom: 6 }}>
                          <div>
                            <div style={{ fontSize: 12, fontWeight: 600 }}>{r.aciklama}</div>
                            <div style={{ fontSize: 11, color: 'var(--text3)' }}>{fmtDate(r.tarih)}</div>
                          </div>
                          <div style={{ fontWeight: 700, color: 'var(--green)', fontFamily: 'var(--font-mono)' }}>{fmt(r.tutar)}</div>
                        </div>
                      ))}
                    </>
                  )}
                  {(odemeOzetModal.kart_liste || []).length > 0 && (
                    <>
                      <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--text3)', marginTop: 12, marginBottom: 8 }}>💳 KART ÖDEMELERİ</div>
                      {odemeOzetModal.kart_liste.map((r, i) => (
                        <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 10px', borderRadius: 6, background: 'var(--bg3)', marginBottom: 6 }}>
                          <div>
                            <div style={{ fontSize: 12, fontWeight: 600 }}>{r.aciklama}</div>
                            <div style={{ fontSize: 11, color: 'var(--text3)' }}>{fmtDate(r.tarih)} · {r.banka} {r.kart_adi}</div>
                          </div>
                          <div style={{ fontWeight: 700, color: '#4a9eff', fontFamily: 'var(--font-mono)' }}>{fmt(r.tutar)}</div>
                        </div>
                      ))}
                    </>
                  )}
                  {!(odemeOzetModal.nakit_liste?.length) && !(odemeOzetModal.kart_liste?.length) && (
                    <div style={{ textAlign: 'center', padding: 32, color: 'var(--text3)' }}>Bu ay kayıt yok</div>
                  )}
                </>
              ) : (
                /* Anlık, Sabit, Nakit, Kart — tekli liste */
                <>
                  {(odemeOzetModal.liste || []).length === 0 ? (
                    <div style={{ textAlign: 'center', padding: 32, color: 'var(--text3)' }}>Bu ay kayıt yok</div>
                  ) : (
                    <>
                      {/* Toplam */}
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 12px', borderRadius: 8, background: 'var(--bg2)', marginBottom: 12, border: '1px solid var(--border)' }}>
                        <span style={{ fontSize: 12, color: 'var(--text3)', fontWeight: 600 }}>TOPLAM</span>
                        <span style={{ fontWeight: 700, fontSize: 16, fontFamily: 'var(--font-mono)' }}>
                          {fmt((odemeOzetModal.liste || []).reduce((s, r) => s + parseFloat(r.tutar || 0), 0))}
                        </span>
                      </div>
                      {(odemeOzetModal.liste || [])
                        .sort((a, b) => new Date(b.tarih) - new Date(a.tarih))
                        .map((r, i) => (
                          <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 10px', borderRadius: 6, background: 'var(--bg3)', marginBottom: 6 }}>
                            <div style={{ flex: 1 }}>
                              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                <span style={{ fontSize: 12, fontWeight: 600 }}>{r.aciklama || r.gider_adi || '—'}</span>
                                {r.kaynak && <span style={{ fontSize: 10, color: 'var(--text3)', background: 'var(--bg2)', padding: '1px 6px', borderRadius: 4 }}>{r.kaynak}</span>}
                                {r.odeme_yontemi === 'kart' && <span style={{ fontSize: 10, color: '#4a9eff' }}>💳 Kart</span>}
                                {r.kategori && <span style={{ fontSize: 10, color: 'var(--text3)' }}>{r.kategori}</span>}
                              </div>
                              <div style={{ fontSize: 11, color: 'var(--text3)', marginTop: 2 }}>
                                {fmtDate(r.tarih)}
                                {r.banka && ` · ${r.banka} ${r.kart_adi || ''}`}
                              </div>
                            </div>
                            <div style={{ fontWeight: 700, fontFamily: 'var(--font-mono)', color: r.odeme_yontemi === 'kart' ? '#4a9eff' : 'var(--red)' }}>
                              {fmt(r.tutar)}
                            </div>
                          </div>
                        ))
            ...
            <div className="modal-footer">
              <button className="btn btn-secondary" onClick={() => setOdemeOzetModal(null)}>Kapat</button>
            </div>
          </div>
        </div>
      )}
