...
export default function Panel({ onNavigate }) {
  ...
  return (
    <div className="page">
      ...
      {/* Karar motoru uyarıları — nakit riski, simülasyon alarmları */}
      {panel.kararlar?.filter(k => k.seviye === 'KRITIK' || k.seviye === 'UYARI').length > 0 && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 16 }}>
          {panel.kararlar.filter(k => k.seviye === 'KRITIK' || k.seviye === 'UYARI').map((k, i) => {
            const kritik = k.seviye === 'KRITIK';
            const renk = kritik ? 'var(--red)' : k.renk === 'TURUNCU' ? '#f07040' : 'var(--yellow)';
            const bg = kritik ? 'rgba(220,50,50,0.07)' : 'rgba(220,160,0,0.05)';
            return (
              <div key={i} className={k.blink ? 'blink' : ''} style={{
                background: bg,
                border: `1px solid ${renk}`,
                borderLeft: `3px solid ${renk}`,
                borderRadius: 8, padding: '10px 16px',
              }}>
                <div style={{ fontWeight: 700, fontSize: 12, color: renk }}>{k.baslik}</div>
                <div style={{ fontSize: 11, color: 'var(--text2)', marginTop: 3 }}>{k.mesaj}</div>
                {k.aksiyon && (
                  <div style={{ fontSize: 11, color: 'var(--text3)', marginTop: 2 }}>→ {k.aksiyon}</div>
                )}
              </div>
            );
          })}
