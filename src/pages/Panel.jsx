...
export default function Panel({ onNavigate }) {
  ...
  const [acikOdemeler, setAcikOdemeler] = useState(new Set()); // accordion
        ...
        <div className="card">
          ...
          {tumUyarilar.length === 0 ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8, maxHeight: 320, overflowY: 'auto' }}>
              {tumUyarilar.map((u, i) => {
                const kritik = u.seviye === 'KRITIK';
                const renk = kritik ? 'var(--red)' : u.renk === 'TURUNCU' ? '#f07040' : 'var(--yellow)';
                const bg = kritik ? 'rgba(220,50,50,0.08)' : u.renk === 'TURUNCU' ? 'rgba(240,112,64,0.07)' : 'rgba(220,160,0,0.05)';
                const acik = kritik || acikOdemeler.has(i);
                const toggle = () => {
                  if (kritik) return;
                  setAcikOdemeler(prev => {
                  ...
                  <div key={i} className={u.blink ? 'blink' : ''} style={{
                    ...
                    <div onClick={toggle} style={{
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
                        <div style={{ fontSize: 13, fontWeight: 700, fontFamily: 'var(--font-mono)', color: renk }}>{fmt(u.tutar)}</div>
                        {!kritik && <span style={{ fontSize: 10, color: 'var(--text3)' }}>{acik ? '▲' : '▼'}</span>}
                      </div>
                    </div>
                    {/* Detay — sadece açıksa veya kritikse */}
                    {acik && (
