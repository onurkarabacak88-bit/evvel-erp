...
export default function Panel({ onNavigate }) {
  ...
  // Vadeli alımlar üstteki uyarı kartında gösteriliyor — listede tekrar çıkmasın
  const uyarilarFiltreli = uyarilar.filter(u => u.kural !== 6);
  const kritikler = uyarilarFiltreli.filter(u => u.seviye === 'KRITIK');
  const diger = uyarilarFiltreli.filter(u => u.seviye !== 'KRITIK');
