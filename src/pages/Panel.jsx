  ...
  async function kismiOdemeOnayla() {
    ...
    try {
      await api(`/odeme-plani/${kismiModal.odemeId}/kismi-ode`, {
        method: 'POST',
        body: { odenen_tutar: odenen, kalan_vade_tarihi: kismiTarih }
      });
              ...
              <div key={i} className={animClass} style={{
                ...
                <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
                  ...
                  <button className="btn btn-ghost btn-sm" disabled={loadingBtn}
                    onClick={() => { setKismiModal({odemeId: u.odeme_id, aciklama: u.aciklama, toplam: u.tutar}); setKismiTutar(''); setKismiTarih(''); }}>✂ Kısmi</button>
                </div>
              </div>
            );
          })}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 12, marginBottom: 16 }}>
        {[
          ...
          (() => {
            ...
            return { label: '🛒 Vadeli Alım (Ödenen)', value: fmt(vadeliOzet.toplam_odenen || 0), sub: subText, renk: subRenk, page: 'vadeli-alimlar' };
          })(),
        ...
        <div className="modal-overlay" onClick={e => e.target === e.currentTarget && setOdemeModal(null)}>
          <div className="modal" style={{ maxWidth: 400 }}>
            ...
            <div className="modal-footer">
              ...
              <button className="btn btn-primary" onClick={() => odemeOnayla(odemeModal.odeme_id, null)}>
                ✓ Evet, Ödendi
              </button>
            </div>
          </div>
      ...
      {kismiModal && (
        <div className="modal-overlay" onClick={e => e.target === e.currentTarget && setKismiModal(null)}>
          <div className="modal" style={{ maxWidth: 420 }}>
            ...
            <div className="modal-body">
              <div style={{ background: 'var(--bg3)', borderRadius: 8, padding: '10px 14px', marginBottom: 16 }}>
                <div style={{ fontSize: 12, fontWeight: 600 }}>{kismiModal.aciklama}</div>
                <div style={{ fontSize: 11, color: 'var(--text3)', marginTop: 4 }}>
                  Toplam borç: <strong>{parseInt(kismiModal.toplam).toLocaleString('tr-TR')} ₺</strong>
                </div>
              </div>
              <div className="form-group">
                ...
                <input
                  type="number"
                  placeholder={`0 - ${parseInt(kismiModal.toplam)} arası`}
                  value={kismiTutar}
            ...
            <div className="modal-footer">
              ...
              <button className="btn btn-primary"
                disabled={loadingBtn || !kismiTutar || !kismiTarih}
