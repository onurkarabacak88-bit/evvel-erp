...
STOK_KEYS = (
    ...
    "soda_adet",
    "cookie_adet",
    "pasta_adet",   # toplam pasta (eski uyumluluk; bireysel kalemler pasta_* ile gelir)
    "sut_litre",
    "surup_adet",
    "kahve_paket",
    "karton_bardak",
    ...
    "pecete_paket",
    "diger_sarf",
    # ── Pastalar (Excel listesi) ──────────────────────────────────
    # Porsiyon Tatlılar
    "pasta_porsiyon_sade",
    "pasta_porsiyon_antep",
    "pasta_porsiyon_cik",
    # Magnolya
    "pasta_mag_cilek",
    "pasta_mag_lotus",
...
PASTA_GRUPLAR: List[Tuple[str, List[str]]] = [
    ...
    ("Magnolya", ["pasta_mag_cilek", "pasta_mag_lotus"]),
    ("Adet Tatlılar", [
        "pasta_buyuk_tart", "pasta_kucuk_tart", "pasta_snickers", "pasta_malaga",
        "pasta_latte", "pasta_muzlu_rulo", "pasta_cik_rulo", "pasta_meyveli_rulo",
        "pasta_browni", "pasta_dilim_ss_sade", "pasta_cream_puff", "pasta_kavala",
    ]),
    ("Cup Tatlılar", [
        "pasta_cup_limon", "pasta_cup_yerfistik", "pasta_cup_cilek", "pasta_cup_karamel",
        "pasta_cup_lotus", "pasta_cup_antep", "pasta_cup_hindistan", "pasta_profiterol",
    ]),
    ("Kare Cheesecake", ["pasta_kare_cik", "pasta_kare_yerfistik", "pasta_kare_karamel", "pasta_kare_limon"]),
    ("Dilim San Sebastian", ["pasta_dilim_sade", "pasta_dilim_antep", "pasta_dilim_cik", "pasta_dilim_yaban"]),
]
...
STOK_LABEL_TR = {
    ...
    # Pastalar
    "pasta_porsiyon_sade": "San Sebastian (Porsiyon)",
    "pasta_porsiyon_antep": "Antep Fıstıklı San Sebastian (Porsiyon)",
    "pasta_porsiyon_cik": "Çikolatalı San Sebastian (Porsiyon)",
    "pasta_mag_cilek": "Magnolya Çilekli",
    "pasta_mag_lotus": "Magnolya Lotuslu",
    "pasta_buyuk_tart": "Büyük Tart",
    "pasta_kucuk_tart": "Küçük Tart",
    "pasta_snickers": "Snickers",
    "pasta_malaga": "Malaga",
    "pasta_latte": "Latte Pasta",
    "pasta_muzlu_rulo": "Muzlu Rulo",
    "pasta_cik_rulo": "Çikolatalı Rulo",
    "pasta_meyveli_rulo": "Meyveli Beyaz Çikolatalı Rulo",
    "pasta_browni": "Browni",
    "pasta_dilim_ss_sade": "Dilim Sade San Sebastian",
    "pasta_cream_puff": "Cream Puff",
    "pasta_kavala": "Kavala Kurabiye",
    "pasta_cup_limon": "Limonlu Cup",
    "pasta_cup_yerfistik": "Yer Fıstıklı Cup",
    "pasta_cup_cilek": "Çilekli Cup",
    "pasta_cup_karamel": "Karamelli Cup",
    "pasta_cup_lotus": "Lotuslu Cup",
    "pasta_cup_antep": "Antep Fıstıklı Cup",
    "pasta_cup_hindistan": "Hindistan Cevizli Cup",
    "pasta_profiterol": "Profiterol",
    "pasta_kare_cik": "Kare Cheesecake Çikolatalı",
    "pasta_kare_yerfistik": "Kare Cheesecake Yer Fıstıklı Karamelli",
    "pasta_kare_karamel": "Kare Cheesecake Karamelli",
    "pasta_kare_limon": "Kare Cheesecake Limonlu",
    "pasta_dilim_sade": "Dilim San Sebastian Sade",
    "pasta_dilim_antep": "Dilim San Sebastian Antep Fıstıklı",
    "pasta_dilim_cik": "Dilim San Sebastian Çikolatalı",
    "pasta_dilim_yaban": "Dilim San Sebastian Yaban Mersinli",
}
...
def _stok_key_from_urun_ad(urun_ad: Any) -> Optional[str]:
    ...
    if "cookie" in n:
        return "cookie_adet"
    # Pastalar — önce spesifik eşleşmeler, sonra genel
    if "porsiyon" in n and "antep" in n:
        return "pasta_porsiyon_antep"
    if "porsiyon" in n and ("çikolata" in n or "cikol" in n):
        return "pasta_porsiyon_cik"
    if "porsiyon" in n or ("san sebast" in n and "dilim" not in n and "kare" not in n):
        return "pasta_porsiyon_sade"
    if "magnolya" in n and "cilek" in n:
        return "pasta_mag_cilek"
    if "magnolya" in n and "lotus" in n:
        return "pasta_mag_lotus"
    if "buyuk tart" in n or "büyük tart" in n:
        return "pasta_buyuk_tart"
    if "kucuk tart" in n or "küçük tart" in n:
        return "pasta_kucuk_tart"
    if "snickers" in n:
        return "pasta_snickers"
    if "malaga" in n:
        return "pasta_malaga"
    if "latte" in n:
        return "pasta_latte"
    if "muzlu rulo" in n:
        return "pasta_muzlu_rulo"
    if "çikolatal" in n and "rulo" in n:
        return "pasta_cik_rulo"
    if "meyveli" in n and "rulo" in n:
        return "pasta_meyveli_rulo"
    if "browni" in n or "brownie" in n:
        return "pasta_browni"
    if "cream puff" in n:
        return "pasta_cream_puff"
    if "kavala" in n:
        return "pasta_kavala"
    if "limonlu cup" in n or ("cup" in n and "limon" in n):
        return "pasta_cup_limon"
    if "yer fistik" in n and "cup" in n:
        return "pasta_cup_yerfistik"
    if "çilek" in n and "cup" in n:
        return "pasta_cup_cilek"
    if "karamel" in n and "cup" in n:
        return "pasta_cup_karamel"
    if "lotus" in n and "cup" in n:
        return "pasta_cup_lotus"
    if "antep" in n and "cup" in n:
        return "pasta_cup_antep"
    if "hindistan" in n and "cup" in n:
        return "pasta_cup_hindistan"
    if "profiterol" in n:
        return "pasta_profiterol"
    if "kare" in n and "çikolata" in n:
        return "pasta_kare_cik"
    if "kare" in n and "yer fistik" in n:
        return "pasta_kare_yerfistik"
    if "kare" in n and "karamel" in n:
        return "pasta_kare_karamel"
    if "kare" in n and "limon" in n:
        return "pasta_kare_limon"
    if "dilim" in n and "antep" in n:
        return "pasta_dilim_antep"
    if "dilim" in n and "çikolata" in n:
        return "pasta_dilim_cik"
    if "dilim" in n and "yaban" in n:
        return "pasta_dilim_yaban"
    if "dilim" in n and "sade" in n:
        return "pasta_dilim_sade"
    if "dilim" in n and "san sebast" in n:
        return "pasta_dilim_ss_sade"
    if "pasta" in n:
        return "pasta_adet"
...
def depo_kalem_kodu_panel_katalog(urun_id: str, urun_ad: str) -> str:
    """
    Panel veya merkez katalog satırı için ``sube_depo_stok.kalem_kodu`` tek doğrusu.

    Bardak / su / şurup vb. genel havuzlar ``STOK_KEYS`` ile birleşir; ürün adında
    «kahve» geçen çeşitler için ``siparis_urun.id`` (``urun_id``) ayrı satır kalır.

    ``urun_id`` boşsa boş string döner.
    """
    uid = str(urun_id or "").strip()
    if not uid:
        return ""
    sk = _stok_key_from_urun_ad(urun_ad)
    if sk and sk != "kahve_paket":
        return sk
    return uid

def depo_kalem_kodu_resolve(cur: Any, urun_id: str, urun_ad_fallback: str = "") -> str:
    """
    ...
    if not uid:
        return ""
    ad_src = str(urun_ad_fallback or "").strip()
    try:
        cur.execute(
            """
...
_PATTERN_ETIKET: Dict[str, str] = {
    ...
    "KAPANIS_KASA_FARK":  "Kapanış kasa mutabakat farkı",
...
def siparis_rezerve_kaynak_depoya_tasi(
    cur: Any,
    *,
    eski_kaynak_depo_sube_id: Optional[str],
    yeni_kaynak_depo_sube_id: str,
    tahsis_adet_map: Dict[str, int],
    kalem_adi_map: Dict[str, str],
) -> None:
    """
    Tahsis ile birikmiş rezerve_adet hangi kaynakta tutuluyorsa (merkez veya şube deposu),
    sipariş çıkış deposu bu şubeye sabitlendiğinde rezervi aynı miktarda yeni depoya taşır.
    merkez_tahsis_yap içindeki kaynak-değiştirme döngüsü ile uyumlu; ek iş kuralı eklemez.
    """
    eski = (eski_kaynak_depo_sube_id or "").strip() or None
    ...
    for kk_raw, adet_raw in tahsis_adet_map.items():
        ...
        if not kk:
            continue
        ta = max(0, int(adet_raw or 0))
        if ta <= 0:
            continue
        kalem_adi = (kalem_adi_map.get(kk) or kk).strip() or kk
        if eski:
            cur.execute(
                """
...
def sube_kabul_kaydet(cur: Any, siparis_talep_id: str, sube_id: str,
    ...
    cur.execute(
        ...
        WHERE id=%s
        """,
        (yeni_talep_durum, sevk_durum, sevk_durum, siparis_talep_id),
    )
    if uyumsuz_satirlar:
        _disiplin_olay_yaz(
            cur,
...
def siparis_akis_ozet(cur: Any, limit: int = 50) -> List[Dict[str, Any]]:
    """Son N siparişin uçtan uca durumu. Şube→Depo→Kabul zinciri dahil."""
    cur.execute(
        """
        ...
        LEFT JOIN subeler sd  ON sd.id=st.sevkiyat_sube_id
        LEFT JOIN subeler sd2 ON sd2.id=st.hedef_depo_sube_id
    ...
    for r in rows:
        ...
        if isinstance(kalem_durumlari, str):
            try:
                ...
                kalem_durumlari = []
        cur.execute(
            "SELECT kalem_kodu, kalem_adi, sevk_adet, kabul_adet, kabul_ts, durum FROM stok_yolda WHERE siparis_talep_id=%s",
            (r.get("id"),),
        )
        yolda = []
        for y in cur.fetchall():
            yolda.append({
                ...
                "sevk_adet": y.get("sevk_adet"),
                "kabul_adet": y.get("kabul_adet"),
        ...
        if yolda:
            ...
            if all(d in ("kabul_edildi", "uzlasildi") for d in durumlar):
                kabul_durum = "kabul_tam"
            elif any(d == "kabul_uyusmazlik" for d in durumlar):
                kabul_durum = "kabul_uyusmazlik"
            elif any(d == "kabul_edildi" for d in durumlar):
                kabul_durum = "kabul_kismi"
        kabul_ts_list = [y.get("kabul_ts") for y in yolda if y.get("kabul_ts")]
        kabul_ts = max(kabul_ts_list) if kabul_ts_list else ""
        sonuc.append({
            ...
            "tahsis_durum": r.get("tahsis_durum"),
            "tahsis_ts": str(r.get("tahsis_ts") or ""),
            "tahsis_yapan_ad": r.get("tahsis_yapan_ad"),
            "sevkiyat_durumu": r.get("sevkiyat_durumu"),
            "sevkiyat_ts": str(r.get("sevkiyat_ts") or ""),
            "sevkiyat_personel_ad": r.get("sevkiyat_personel_ad"),
            "sevk_sube_adi": r.get("sevk_sube_adi"),
            ...
            "kabul_ts": str(kabul_ts) if kabul_ts else "",
        })
    return sonuc
