...
def karar_motoru():
    ...
    with db() as (conn, cur):
        ...
        for v in cur.fetchall():
            ...
            kararlar.append({
                "kural": 6, "seviye": "UYARI" if gun_kaldi > 1 else "KRITIK",
                "renk": "TURUNCU" if gun_kaldi > 1 else "KIRMIZI",
...
def aylik_odeme_plani_uret(yil=None, ay=None):
    ...
    with db() as (conn, cur):
        ...
        for g in cur.fetchall():
            ...
            pid = str(_uuid.uuid4())
            cur.execute("""
                INSERT INTO odeme_plani (id, kart_id, tarih, odenecek_tutar, asgari_tutar, aciklama, durum, kaynak_tablo, kaynak_id)
                SELECT %s, NULL, %s, %s, %s, %s, 'bekliyor', 'sabit_giderler', %s
                WHERE NOT EXISTS (
        ...
        for p in cur.fetchall():
            ...
            cur.execute("""
                INSERT INTO odeme_plani (id, kart_id, tarih, odenecek_tutar, asgari_tutar, aciklama, durum, kaynak_tablo, kaynak_id)
                SELECT %s, NULL, %s, %s, %s, %s, 'bekliyor', 'personel', %s
                WHERE NOT EXISTS (
                    SELECT 1 FROM odeme_plani
                    WHERE kaynak_id = %s
                    AND DATE_TRUNC('month', tarih) = DATE_TRUNC('month', %s::date)
...
def uyari_motoru():
    ...
    with db() as (conn, cur):
        ...
        # Vadeli alımlar — 7 gün içinde vadesi gelenler
        cur.execute("""
            SELECT * FROM vadeli_alimlar WHERE durum='bekliyor'
            AND vade_tarihi BETWEEN %s AND %s
        """, (bugun, bugun + timedelta(days=7)))
        for v in cur.fetchall():
            ...
            uyarilar.append({
                "kural": 6,
                "seviye": "KRITIK" if gun_kalan <= 1 else "UYARI",
                "renk": "KIRMIZI" if gun_kalan <= 1 else "TURUNCU",
                "baslik": f"Vadeli Alım: {v['aciklama']}",
                "mesaj": f"{gun_kalan} gün sonra {fmt(v['tutar'])} ödeme vadesi geliyor.",
                "aciklama": v['aciklama'],
                "tedarikci": v.get('tedarikci') or '',
                "tutar": float(v['tutar']),
                "vade_tarihi": str(v['vade_tarihi']),
