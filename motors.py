...
def odeme_strateji_motoru():
    ...
    with db() as (conn, cur):
        ...
        # Bekleyen ödemeler — kart + kart dışı (sabit gider, personel, borç)
        cur.execute("""
            SELECT op.*,
                   COALESCE(k.banka, '') as banka,
                   COALESCE(k.kart_adi, op.aciklama) as kart_adi,
                   COALESCE(k.faiz_orani, 0) as faiz_orani
            FROM odeme_plani op
            LEFT JOIN kartlar k ON k.id = op.kart_id
            WHERE op.durum='bekliyor' AND op.tarih >= %s
            ORDER BY COALESCE(k.faiz_orani, 0) DESC, op.tarih ASC
        """, (bugun,))
        for o in cur.fetchall():
